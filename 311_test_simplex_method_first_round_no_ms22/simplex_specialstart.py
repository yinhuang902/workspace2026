...
# ----------- CS-only evaluator (first-round optimisation) -----------
def cs_only_on_tetra_for_scene(ms_bundle, tet_vertices, fverts_scene):
    """
    Lightweight variant of ms_on_tetra_for_scene that only computes
    the constant-cut (CS).  MS is skipped entirely.

    Returns the same 4-tuple as ms_on_tetra_for_scene:
        (ms_val, new_pt_ms, c_val, c_pt)
    where ms_val = +inf and new_pt_ms = None are placeholders.
    If CS fails, c_val = +inf (not -inf) so that the simplex is not
    artificially attractive in a min-LB ranking.
    """
    ms_bundle.update_tetra(tet_vertices, fverts_scene)

    ok_c, c_val, c_pt = ms_bundle.solve_const_cut()
    if not ok_c:
        print("c_s solve wrong (cs_only)")
        c_val = float('inf')   # +inf, NOT -inf — keeps failed simplex out of min-LB
        c_pt = None

    return float('inf'), None, c_val, c_pt
...
def evaluate_all_tetra(nodes, scen_values, ms_bundles, first_vars_list,
                       ...
                       cs_only=False):  # when True, skip MS solves (first-round optimisation)
    """
    ...
    """
    print(f"[DBG] evaluate_all_tetra: start (cs_only={cs_only})", flush=True)
    ...
    for k, simp in enumerate(simplices):
        ...
        for w in range(S):
            ...
            # When cs_only=True, never read from the normal cache
            # (we don't want stale full-MS entries to mask the CS-only path,
            #  and CS-only results must not be stored in the cache either).
            hit = (not cs_only) and (cache_on and (ms_cache is not None) and (cache_key in ms_cache))
            ...
            if hit:
                ...
                if cs_only:
                    ms_val, new_pt_ms, c_val, c_pt = cs_only_on_tetra_for_scene(
                ...
                # When cs_only=True, do NOT write to the normal cache.
                # CS-only entries (ms_val=+inf) must never be mistaken for
                # full MS+CS evaluations in later iterations.
                if (not cs_only) and cache_on and (ms_cache is not None):
        ...
        # When cs_only=True, do NOT read/write lb_sur_cache — same rationale as ms_cache.
        if (not cs_only) and lb_sur_cache is not None and key_base in lb_sur_cache:
            ...
            if (not cs_only) and lb_sur_cache is not None:
                lb_sur_cache[key_base] = LB_sur

        if cs_only or not np.isfinite(ms_total):
            best_scene = None
            x_ms_best = None
        else:
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    point_select_mode:
        1 = Mode 1 (default): try ms candidates first, then c_s, then edge midpoint.
        2 = Mode 2 (legacy, disabled): single weighted-composite of ms points.
        3 = Mode 3 (parity-based):
            even iterations: best ms → weighted avg of valid c_s
            odd iterations:  weighted avg c_s → unweighted avg c_s → best c_s → ms
            If all parity-priority candidates fail, fall through to Mode 1 chain.
    """
    ...
    if initial_nodes is not None:
        ...
        if verbose:
            ...
            _psm_names = {1: "Mode 1 (ms→c_s→edge)", 2: "Mode 2 (ms weighted composite)", 3: "Mode 3 (even: ms→avg_cs; odd: avg_cs→avg_cs_uw→best_cs→ms)"}
            print(f"[Init] point_select_mode={point_select_mode}: {_psm_names.get(point_select_mode, 'unknown')}")
    else:
    ...
    # Detect box start (corners_from_var_bounds) vs custom initial nodes.
    # CS-only first-round optimisation is only used for box start.
    is_box_start = (initial_nodes is None)
    ...
    while len(nodes) < target_nodes:
        ...
        try:
            # CS-only first-round optimisation: skip MS for all simplices
            # on iter 0 when using box start.  MS will be computed later
            # only for the selected min-LB simplex.
            _use_cs_only = (it == 0 and is_box_start)
            if _use_cs_only and verbose:
                print(f"[Iter 0] CS-only first-round mode active (box start)")
                sys.stdout.flush()
        ...
        # ============================================================
        # === CS-only first-round backfill: compute MS for the
        #     selected LB simplex only (box-start iter 0)           ===
        # ============================================================
        if _use_cs_only:
            _t_ms_backfill = perf_counter()
            if verbose:
                print(f"[Iter 0 CS-only] Computing MS for selected LB simplex "
                      f"T{lb_simp_idx} across {S} scenarios ...")
                sys.stdout.flush()

            _bf_verts = lb_simp_rec["verts"]
            _bf_idxs  = lb_simp_rec["vert_idx"]
            _bf_fverts = [[scen_values[s][i] for i in _bf_idxs] for s in range(S)]
            _bf_key_base = tuple(sorted(_bf_idxs))
            ...
            for w in range(S):
                ms_val, new_pt_ms, c_val, c_pt = ms_on_tetra_for_scene(
                    ms_bundles[w], _bf_verts, _bf_fverts[w]
                )
                _bf_ms_scene.append(ms_val)
                _bf_xms_scene.append(new_pt_ms)
                _bf_c_scene.append(c_val)
                _bf_cpts_scene.append(c_pt)

                # Store full result in ms_cache so later iterations won't recompute
                cache_key = (int(w), _bf_key_base)
                if ms_cache is not None:
                    ms_cache[cache_key] = (ms_val, new_pt_ms, c_val, c_pt)

            # Update the selected simplex record in-place with real MS data
            lb_simp_rec["ms_per_scene"]  = _bf_ms_scene
            lb_simp_rec["xms_per_scene"] = _bf_xms_scene
            lb_simp_rec["c_per_scene"]   = _bf_c_scene
            lb_simp_rec["c_point_per_scene"] = _bf_cpts_scene

            if MS_AGG == "sum":
                _bf_ms_total = float(np.sum(_bf_ms_scene))
                _bf_c_total  = float(np.sum(_bf_c_scene))
            elif MS_AGG == "mean":
                _bf_ms_total = float(np.mean(_bf_ms_scene))
                _bf_c_total  = float(np.mean(_bf_c_scene))
            else:
                _bf_ms_total = float(np.sum(_bf_ms_scene))
                _bf_c_total  = float(np.sum(_bf_c_scene))

            lb_simp_rec["ms"]    = _bf_ms_total
            lb_simp_rec["c_agg"] = _bf_c_total

            _bf_best_scene = int(np.argmin(_bf_ms_scene))
            lb_simp_rec["best_scene"]      = _bf_best_scene
            lb_simp_rec["x_ms_best_scene"] = _bf_xms_scene[_bf_best_scene]

            # Update ms_b from the now-real MS data
            ms_b = _bf_ms_total
            lb_simp_rec["ms"] = _bf_ms_total

            # NOTE: We intentionally do NOT overwrite lb_simp_rec["LB"] here.
            # The round-0 simplex selection was based on CS-only LB and that
            # decision is final.  The real (MS+CS) LB is computed below for
            # informational / debug purposes only.
            _bf_LB_full = solve_surrogate_lb_for_tet(_bf_fverts, _bf_ms_scene, _bf_c_scene)
            if verbose:
                print(f"[Iter 0 CS-only] MS backfill done for T{lb_simp_idx}: "
                      f"ms_total={_bf_ms_total:.6e}, "
                      f"CS-only LB={lb_simp_rec['LB']:.6e}, "
                      f"full LB={_bf_LB_full:.6e}, "
                      f"time={perf_counter()-_t_ms_backfill:.2f}s")
                sys.stdout.flush()

            _phases["4b_cs_only_ms_backfill"] = perf_counter() - _t_ms_backfill
        ...
        with open(dbg_candidates_path, "a", encoding="utf-8") as _dbf:
            ...
            _dbf.write("\n")
        # --- Point selection mode logic ---
        MODE2 = (point_select_mode == 2)

        new_node = None
        chosen_ms = None
        chosen_cand = None
        _mode3_reason = None       # set by Mode 3 logic; None for Mode 1
        stop_due_to_collision = False
        ...
        if MODE2:
            ...
            if weights:
                ...
                if min_dist_to_nodes(candidate_pt, nodes) >= min_dist:
                    ...
                    # Bug 5 fix: Mode 2 collision now falls through to Mode 1
                    # instead of immediately stopping via handle_collision.
                    if verbose:
                        print(f"[Iter {it}] Mode 2 composite point collided, "
                              f"falling through to Mode 1 individual candidates...")

        # ---------- Mode 1 / Mode 3: try ms candidates, then c_s, then edge midpoint ----------
        if (new_node is None) and (not stop_due_to_collision):
            # Separate ms-sourced and c_s-sourced candidates
            # Bug 3 fix: handle c_s_fallback explicitly — treat as c_s candidates
            ms_cands = [ci for ci in cand_items if ci.get("pt_source", "ms") in ("ms",)]
            cs_cands = [ci for ci in cand_items if ci.get("pt_source", "") in ("c_s", "c_s_fallback")]
            ...
            _selection_source = None   # physical source: "ms", "c_s", "cs_weighted_avg", "collision_edge_midpoint"
            _selection_rank = None     # rank within its group (1-based)
            _mode3_reason = None       # Mode 3 context: "mode3_ms_primary", "mode3_cs_weighted_primary", "mode3_fallback_to_mode1_ms", etc.
            _ms_collisions = 0
            _cs_collisions = 0
            _first_collision_node_idx = None   # Bug 2 fix: track first collision

            # --- Mode 3: parity-based priority order ---
            # Even iterations:
            #   Type A  (best feasible ms) → Type B (weighted avg of valid c_s)
            #
            # Odd iterations:
            #   Type B  (weighted avg of valid c_s)
            #   → Type B2 (unweighted avg of valid c_s)
            #   → Type B3 (best individual valid c_s)
            #   → Type A  (best feasible ms)
            #
            # If all parity-priority candidates are unavailable or collide,
            # fall through to the normal Mode 1 chain:
            #   ms candidates → c_s candidates → edge midpoint fallback
            if point_select_mode == 3:
                # --- Build Type A: best feasible ms candidate ---
                _type_a_node = None
                _type_a_cand = None
                for _a_ci in ms_cands_sorted:
                    _a_pt = _a_ci["cand_pt"]
                    if _a_pt is None:
                        continue
                    if min_dist_to_nodes(_a_pt, nodes) >= min_dist:
                        _a_snapped, _a_lt, _a_li = _snap_feature(_a_pt, _a_ci.get("_rec", None))
                        _a_ci["loc_type"] = _a_lt
                        _a_ci["loc_info"] = _a_li
                        _type_a_node = _a_snapped
                        _type_a_cand = _a_ci
                        break

                # --- Build Type B: CS-weighted-average ---
                _type_b_node = None
                _type_b_cand = None
                _cs_valid = []   # list of (point_array, cs_value, candidate_dict)
                for _b_ci in cs_cands_sorted:
                    # Must belong to the selected LB simplex
                    if int(_b_ci["simplex_index"]) != int(lb_simp_rec["simplex_index"]):
                        continue
                    # Must be c_s-side candidate
                    if _b_ci.get("pt_source") not in ("c_s", "c_s_fallback"):
                        continue
                    # Must have a valid point
                    _b_pt = _b_ci["cand_pt"]
                    if _b_pt is None:
                        continue
                    _b_pt = np.asarray(_b_pt, float)
                    if not np.all(np.isfinite(_b_pt)):
                        continue
                    # Must have a finite, non-inf CS value (screens out infeasible c_s)
                    _b_val = float(_b_ci["cand_ms"])
                    if not np.isfinite(_b_val):
                        continue
                    if math.isinf(_b_val):
                        continue
                    _cs_valid.append((_b_pt, _b_val, _b_ci))

                _n_cs_valid = len(_cs_valid)
                if verbose and _n_cs_valid == 0 and len(cs_cands_sorted) > 0:
                    print(f"[Iter {it}] Mode 3 Type B: 0/{len(cs_cands_sorted)} "
                          f"c_s candidates valid (all infeasible/filtered)")

                if _n_cs_valid == 1:
                    # Single valid c_s point — use it directly
                    _b_pt_single, _b_val_single, _ = _cs_valid[0]
                    if min_dist_to_nodes(_b_pt_single, nodes) >= min_dist:
                        _b_snapped, _b_lt, _b_li = _snap_feature(_b_pt_single, lb_simp_rec)
                        _type_b_node = _b_snapped
                        _type_b_cand = {
                            "simplex_index": lb_simp_rec["simplex_index"],
                            "scene": -1,
                            "cand_ms": _b_val_single,
                            "cand_pt": _b_snapped,
                            "pt_source": "cs_weighted_avg",
                            "_rec": lb_simp_rec,
                            "loc_type": _b_lt,
                            "loc_info": _b_li,
                        }
                elif _n_cs_valid >= 2:
                    # Rank-based exponential weighting: w_i = exp(-alpha * rank_i)
                    # rank 0 = best (smallest CS value)
                    _alpha = 0.5
                    _cs_valid.sort(key=lambda x: x[1])   # sort by CS ascending
                    _wts = [math.exp(-_alpha * rank) for rank in range(_n_cs_valid)]
                    _w_sum = sum(_wts)
                    _wts = [w / _w_sum for w in _wts]
                    _cs_avg = sum(w * pt for w, (pt, _, _) in zip(_wts, _cs_valid))
                    if min_dist_to_nodes(_cs_avg, nodes) >= min_dist:
                        _b_snapped, _b_lt, _b_li = _snap_feature(_cs_avg, lb_simp_rec)
                        _wt_val = sum(w * v for w, (_, v, _) in zip(_wts, _cs_valid))
                        _type_b_node = _b_snapped
                        _type_b_cand = {
                            "simplex_index": lb_simp_rec["simplex_index"],
                            "scene": -1,
                ...
                # --- Build Type B2: unweighted average of valid c_s points ---
                _type_b2_node = None
                _type_b2_cand = None
                if _n_cs_valid >= 2:
                    _uw = 1.0 / _n_cs_valid
                    _cs_avg_uw = sum(pt for pt, _, _ in _cs_valid) * _uw
                    if min_dist_to_nodes(_cs_avg_uw, nodes) >= min_dist:
                        _b2_snapped, _b2_lt, _b2_li = _snap_feature(_cs_avg_uw, lb_simp_rec)
                        _b2_val = sum(v for _, v, _ in _cs_valid) * _uw
                        _type_b2_node = _b2_snapped
                        _type_b2_cand = {
                            "simplex_index": lb_simp_rec["simplex_index"],
                            "scene": -1,
                            "cand_ms": _b2_val,
                            "cand_pt": _b2_snapped,
                            "pt_source": "cs_unweighted_avg",
                            "_rec": lb_simp_rec,
                            "loc_type": _b2_lt,
                            "loc_info": _b2_li,
                        }

                # --- Build Type B3: best individual c_s (smallest value) ---
                _type_b3_node = None
                _type_b3_cand = None
                if _n_cs_valid >= 1:
                    # _cs_valid is sorted by value ascending after Type B, index 0 = best
                    _best_pt, _best_val, _best_ci = _cs_valid[0]
                    if min_dist_to_nodes(_best_pt, nodes) >= min_dist:
                        _b3_snapped, _b3_lt, _b3_li = _snap_feature(_best_pt, lb_simp_rec)
                        _type_b3_node = _b3_snapped
                        _type_b3_cand = {
                            "simplex_index": lb_simp_rec["simplex_index"],
                            "scene": _best_ci.get("scene", -1),
                            "cand_ms": _best_val,
                            "cand_pt": _b3_snapped,
                            "pt_source": "cs_best",
                            "_rec": lb_simp_rec,
                ...
                # Even: Type A (best ms) → Type B (weighted avg c_s)
                # Odd:  Type B (weighted avg c_s) → Type B2 (unweighted avg c_s)
                #       → Type B3 (best individual c_s) → Type A (best ms)
                if it % 2 == 0:
                    _m3_order = [
                        (_type_a_node, _type_a_cand, "mode3_ms_primary"),
                        (_type_b_node, _type_b_cand, "mode3_cs_weighted_secondary"),
                    ]
                else:
                    _m3_order = [
                        (_type_b_node,  _type_b_cand,  "mode3_cs_weighted_primary"),
                        (_type_b2_node, _type_b2_cand, "mode3_cs_unweighted"),
                        (_type_b3_node, _type_b3_cand, "mode3_cs_best"),
                        (_type_a_node,  _type_a_cand,  "mode3_ms_secondary"),
                    ]

                for _m3_nd, _m3_ci, _m3_label in _m3_order:
                    if _m3_nd is not None and _m3_ci is not None:
                        new_node = _m3_nd
                        chosen_ms = _m3_ci["cand_ms"]
                        chosen_cand = _m3_ci
                        # Physical source from the candidate metadata
                        _selection_source = _m3_ci.get("pt_source", "ms")
                        _mode3_reason = _m3_label
                        _selection_rank = 1
                        _m3_friendly = {"mode3_ms_primary": "ms",
                                        "mode3_cs_weighted_primary": "avg_cs",
                                        "mode3_cs_unweighted": "avg_cs_uw",
                                        "mode3_cs_best": "best_cs",
                                        "mode3_ms_secondary": "ms_secondary",
                                        "mode3_cs_weighted_secondary": "avg_cs_secondary"}
                        if verbose:
                            print(
                                f"[Iter {it}] Mode 3 pick: {_m3_friendly.get(_m3_label, _m3_label)} "
                                f"{tuple(map(float, _m3_nd))} "
                                f"val={chosen_ms:.3e} "
                                f"(simp T{_m3_ci['simplex_index']}, "
                                f"source={_selection_source})"
                            )
                        break

                if new_node is None:
                    _mode3_reason = "mode3_fallback_to_mode1"
                    if verbose:
                        print(f"[Iter {it}] Mode 3: no usable parity-ordered candidate, "
                              f"falling back to Mode 1 chain...")

            # Phase 1: try all ms candidates in order (skipped if Mode 3 already chose)
            if new_node is None:
                for rank, ci in enumerate(ms_cands_sorted, start=1):
                    ...
                    if min_dist_to_nodes(cand_pt, nodes) >= min_dist:
                        ...
                        if _mode3_reason == "mode3_fallback_to_mode1":
                            _mode3_reason = "mode3_fallback_to_mode1_ms"
                        ...
                        _ms_collisions += 1
                        if _first_collision_node_idx is None:
                            _dists = np.linalg.norm(np.asarray(nodes, float) - np.asarray(cand_pt, float), axis=1)
                            _first_collision_node_idx = int(np.argmin(_dists))
                        if verbose:
                            print(
            ...
            # Phase 2: if all ms collided, try c_s candidates (skipped if already chosen)
            if new_node is None and cs_cands_sorted:
                ...
                for rank, ci in enumerate(cs_cands_sorted, start=1):
                    ...
                    if min_dist_to_nodes(cand_pt, nodes) >= min_dist:
                        ...
                        if _mode3_reason == "mode3_fallback_to_mode1":
                            _mode3_reason = "mode3_fallback_to_mode1_c_s"
                        _selection_rank = rank
                        if verbose:
                            print(
            ...
            if new_node is None and (_ms_collisions + _cs_collisions) > 0:
                ...
                # Bug 2 fix: use the FIRST collision's node index for edge prioritization,
                # not the last candidate's closest node.
                ...
                # Override collision_node_idx with the first collision's node
                if _first_collision_node_idx is not None:
                    collision_node_idx = _first_collision_node_idx
            ...
            if verbose:
                ...
                if _mode3_reason:
                    sel_info += f", mode3_reason={_mode3_reason}"
                if _ms_collisions > 0:
        ...
        if stop_due_to_collision:
            ...
            # Bug 6 fix: seed RNG for reproducible tie-breaking
            _rng = np.random.default_rng(seed=it)
            ...
            for _fb_group, _fb_list in [("collision_edge", _collision_edges),
                ...
                for _elen, _ei, _ej, _midpt in _fb_list:
                    ...
                    if _fb_dist >= min_dist:
                        ...
                        stop_due_to_collision = False
                        _fallback_used = True
                        # Bug 7 fix: set chosen_cand/chosen_ms for edge midpoint
                        chosen_ms = 0.0
                        chosen_cand = {
                            "simplex_index": _lb_simp_idx,
                            "scene": -1,
                            "cand_ms": 0.0,
                            "cand_pt": _midpt,
                            "pt_source": "edge_midpoint",
                            "_rec": lb_simp_rec,
                            "loc_type": "edge",
                            "loc_info": {"edge_verts": (_vert_idxs[_ei], _vert_idxs[_ej])},
                        }
                        if verbose:
        ...
        _t_phase = perf_counter()
        if chosen_cand is not None:
            _pt_src = chosen_cand.get("pt_source", "unknown")
            # Prefer Mode 3 reason label when available; otherwise short physical source
            if _mode3_reason:
                _src_label = {"mode3_ms_primary": "ms",
                              "mode3_cs_weighted_primary": "avg_cs",
                              "mode3_cs_unweighted": "avg_cs_uw",
                              "mode3_cs_best": "best_cs",
                              "mode3_ms_secondary": "fb_ms",
                              "mode3_cs_weighted_secondary": "fb_avg_cs",
                              "mode3_fallback_to_mode1_ms": "fb_ms",
                              "mode3_fallback_to_mode1_c_s": "fb_c_s"}.get(_mode3_reason, _mode3_reason)
            else:
                _src_label = {"ms": "ms", "c_s": "c_s", "cs_weighted_avg": "avg_cs",
                              "c_s_fallback": "c_s", "edge_midpoint": "edge_mid"}.get(_pt_src, _pt_src)
            selection_reason_hist.append(_src_label)
