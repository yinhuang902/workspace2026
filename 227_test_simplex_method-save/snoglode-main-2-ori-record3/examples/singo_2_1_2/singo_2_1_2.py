"""
singo_2_1_2.py — Snoglode example for Julia SINGO Global/2_1_2
...
Stage split (nfirst=2, JuMP column order):
  ...
  Second-stage: x3 (col 3), x4 (col 4), x5 (col 5), x6 (col 6)

Stochastic perturbation:
  linconstr[1]: 6x1+3x2+3x3+2x4+x5 <= 6.5  (has 2nd-stage vars x3,x4,x5)
  linconstr[2]: 10x1+10x3+x6 <= 20           (has 2nd-stage but nmodified=2 >= nparam=2 -> break)
  Only c1 RHS modified: addnoise(6.5) -> 6.5 * U(0.5, 2.0)

Objective:
  Min -0.5*(x1^2+x2^2+x3^2+x4^2+x5^2) - 10.5*x1 - 7.5*x2 - 3.5*x3 - 2.5*x4 - 1.5*x5 - 10*x6

Verification at x=(0,...,0): obj=0, c1=0<=6.5, c2=0<=20 (feasible)
"""
import os
import sys
...
scenario_c1_rhs = [addnoise(6.5, _rng) for _ in range(num_scenarios)]
...
def subproblem_creator(scenario_name):
    ...
    m.x1 = pyo.Var(bounds=(0, 1), initialize=0)     # col 1
    ...
    m.x5 = pyo.Var(bounds=(0, 1), initialize=1)     # col 5, start=1
    m.x6 = pyo.Var(bounds=(0, None), initialize=20) # col 6, x6>=0, start=20

    # Constraints
    rhs = scenario_c1_rhs[scen_idx]
    m.c1 = pyo.Constraint(expr=6*m.x1 + 3*m.x2 + 3*m.x3 + 2*m.x4 + 1*m.x5 <= rhs)
    m.c2 = pyo.Constraint(expr=10*m.x1 + 10*m.x3 + m.x6 <= 20)

    # Objective
    m.obj = pyo.Objective(
        expr=-0.5*(m.x1**2 + m.x2**2 + m.x3**2 + m.x4**2 + m.x5**2)
...
if __name__ == "__main__":
    ...
    if size == 1:
        ...
        params.set_logging(fname=os.getcwd() + "/logs/singo_2_1_2_log")
    if rank == 0:
        params.display()
