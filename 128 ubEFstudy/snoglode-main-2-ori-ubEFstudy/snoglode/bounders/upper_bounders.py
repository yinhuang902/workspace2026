...
class AbstractCandidateGenerator(BoundingProblemBase):
    ...
    def generate(self, 
        ...
        assert type(node)==Node
        assert type(subproblems)==Subproblems
        result = self.generate_candidate(node, subproblems)
        
        # Store EF diagnostic info on node if returned (tuple of length 6)
        if len(result) == 6:
            candidate_found, candidate_solution, candidate_obj, ef_used, ef_time_s, ef_stop_reason = result
            node.ub_problem.ef_used = ef_used
            node.ub_problem.ef_time_s = ef_time_s
            node.ub_problem.ef_stop_reason = ef_stop_reason
            return candidate_found, candidate_solution, candidate_obj
        else:
            # Non-EF generators return 3 values, EF fields stay at defaults
            return result
...
class AverageLowerBoundSolution(AbstractCandidateGenerator):
    ...
    def generate_candidate(self, 
        ...
        """
        # Return 3-tuple (non-EF generator)
        return (True, 
...
class SolveExtensiveForm(AbstractCandidateGenerator):
    ...
    def generate_candidate(self, 
        ...
        Returns 6-tuple: (candidate_found, candidate_solution, obj, ef_used, ef_time_s, ef_stop_reason)
        """
        ...
        # Stop timing and capture termination info
        ef_time_s = time.perf_counter() - ef_start
        try:
            ef_stop_reason = str(ef_results.solver.termination_condition)
        except Exception:
            ef_stop_reason = "unknown"
        ...
        if (size > 1):
            ...
            if (num_sols == 0):
                ...
                return False, None, math.nan, True, ef_time_s, ef_stop_reason
            ...
            # return info
            subproblems.ef.deactivate()
            return True, sol, best_obj, True, ef_time_s, ef_stop_reason
        ...
        if ef_results.solver.termination_condition==TerminationCondition.optimal and \
            ef_results.solver.status==SolverStatus.ok:
                ...
                for obj in subproblems.ef.model.component_objects(pyo.Objective):
                    subproblems.ef.deactivate()
                    return True, subproblems.ef.save_solution(), pyo.value(obj), True, ef_time_s, ef_stop_reason
