...
class Solver():
    ...
    def solve(self, 
        ...
        while (not self.tree.converged(self.runtime)) and (self.iteration <= max_iter):
            ...
            # Store current node for display_status to access EF info
    ...
    def display_status(self, 
        ...
        # EF diagnostic line
        if hasattr(self, '_current_node') and self._current_node is not None:
            ef_on = getattr(self._current_node.ub_problem, 'ef_used', False)
            ef_time = getattr(self._current_node.ub_problem, 'ef_time_s', '-')
            ef_stop = getattr(self._current_node.ub_problem, 'ef_stop_reason', '-')
            if isinstance(ef_time, float):
                ef_time = f"{ef_time:.4f}"
            print(f"  EF: on={ef_on} | time={ef_time}s | stop={ef_stop}")