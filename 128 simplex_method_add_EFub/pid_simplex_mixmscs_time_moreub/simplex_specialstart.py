...
from ub_logging import create_ub_logger, UBLogger
from ef_solve import solve_ef_with_ipopt
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    # === Initialize UB JSONL logger ===
    ub_logger = create_ub_logger(
        root_dir="runs",
        dimension=len(first_vars_list[0]) if first_vars_list else 3,
        scenario_count=S,
        seed=None,  # Add seed if available
        ef_solver_config=None,  # Will be populated when EF solver is added
    )
    ...
    while len(nodes) < target_nodes:
        ...
        # Initialize avg_cs tracking variables for logging
        x_avg = None
        ub_avg_val = None
        avg_cs_accepted = False
        ...
        if valid_c_pts:
            ...
            # Update UB if this improves it (no LB check - true obj can be below surrogate LB)
            if ub_avg_val < UB_global:
                ...
                avg_cs_accepted = True
                if verbose:
                    print(f"[Iter {it}] NEW: avg c_s UB candidate improved UB: {ub_avg_val:.6e} (added to library)")

        # === NEW: EF UB candidate (solve true objective over LB simplex) ===
        ef_simplex_verts = lb_simp_rec.get("verts", None)
        ef_ok = False
        ef_x = None
        ef_ub_val = None
        ef_solve_info = {}
        ...
        if ef_simplex_verts is not None:
            ef_ok, ef_x, ef_ub_val, ef_solve_info = solve_ef_with_ipopt(
                model_list=model_list,
                first_vars_list=first_vars_list,
                simplex_verts=ef_simplex_verts,
                run_dir=ub_logger.run_dir,
                iteration=it,
                time_limit=60.0,
            )
            
            if ef_ok and ef_ub_val is not None:
                # EF gives true objective - no LB check since true obj can be below surrogate LB
                if ef_ub_val < UB_global:
                    UB_global = ef_ub_val
                    UB_node = tuple(ef_x)
                    ub_idx = None
                    ub_candidate_library.append((tuple(ef_x), ef_ub_val))
                    ef_accepted = True
                    if verbose:
                        print(f"[Iter {it}] EF candidate improved UB: {ef_ub_val:.6e}")

        # === Check all candidates in the library to ensure UB never increases ===
        for lib_pt, lib_ub_val in ub_candidate_library:
            if lib_ub_val < UB_global:
        ...
        # ======= Consistency note: surrogate LB can exceed true UB =======
        # When using true objective evaluations (EF, avg c_s), this is expected
        # since the surrogate is a relaxation. Changed from error to warning.
        if LB_global > UB_global + 1e-8:
            if verbose:
                print(f"[Iter {it}] Note: surrogate LB ({LB_global:.6e}) > true UB ({UB_global:.6e}) "
                      f"- this is expected when EF/avg_cs finds better solutions")
        # =======================================================================
        ...
        # === JSONL UB logging (no extra prints to main table) ===
        # Build UB candidates list for this iteration
        ub_candidates = []
        
        # Pre-compute AVG_CS values for logging (explicit init, no dir() checks)
        # These were computed earlier in the iteration if valid_c_pts existed
        avg_cs_enabled = len(valid_c_pts) > 0 if 'valid_c_pts' in dir() else False
        avg_cs_x = x_avg if (avg_cs_enabled and 'x_avg' in dir()) else None
        avg_cs_ub_value = ub_avg_val if (avg_cs_enabled and 'ub_avg_val' in dir()) else None
        avg_cs_accepted = (UB_node == avg_cs_x) if avg_cs_x is not None else False
        
        # Candidate 1: mesh node UB (from f_sum_per_node)
        mesh_ub_node_idx = int(np.argmin(f_sum_per_node))
        mesh_ub_val = float(f_sum_per_node[mesh_ub_node_idx])
        ub_candidates.append({
            "source": "MESH_NODE",
            "enabled": True,
            "ub_value": mesh_ub_val,
            "x": list(nodes[mesh_ub_node_idx]),
            "accepted": (UB_node == tuple(nodes[mesh_ub_node_idx])),
            "reject_reason": None if (UB_node == tuple(nodes[mesh_ub_node_idx])) else "not_best",
        })
        
        # Candidate 2: AVG_CS (average c_s point)
        if avg_cs_enabled:
            ub_candidates.append({
                "source": "AVG_CS",
                "enabled": True,
                "ub_value": avg_cs_ub_value,
                "x": list(avg_cs_x) if avg_cs_x else None,
                "accepted": avg_cs_accepted,
                "reject_reason": None if avg_cs_accepted else "not_best_or_lb_violation",
            })
            ...
            ub_candidates.append({
                ...
                "x": None,
                "accepted": False,
                "reject_reason": "no_valid_c_pts",
            })
        
        # Candidate 3: EF (already solved earlier, use those results for logging)
        # Determine reject reason for logging
        ef_reject_reason = None
        if not ef_ok:
            ef_reject_reason = ef_solve_info.get("message", "solve_failed") if ef_solve_info else "not_attempted"
        elif ef_ub_val is None:
            ef_reject_reason = "no_solution"
        elif not ef_accepted:
            ef_reject_reason = "not_best"
        
        # Add EF candidate with full solver diagnostics
        ub_candidates.append({
            "source": "EF",
            "enabled": ef_simplex_verts is not None,
            "ub_value": ef_ub_val,
            "x": list(ef_x) if ef_x else None,
            "accepted": ef_accepted,
            "reject_reason": ef_reject_reason,
            "solver": ef_solve_info,  # Contains time_sec, termination_condition, status, message, ipopt_exit
        })

        
        # Log EF diagnostics to human-readable text file
        ub_logger.log_ef_text(
            iteration=it,
            enabled=True,
            ef_info={
                "time_sec": ef_solve_info.get("time_sec"),
                "termination_condition": ef_solve_info.get("termination_condition"),
                "status": ef_solve_info.get("status"),
                "ipopt_exit": ef_solve_info.get("ipopt_exit"),
                "ub_value": ef_ub_val,
                "x": ef_x,
                "accepted": ef_accepted,
                "reject_reason": ef_reject_reason,
            },
            simplex_id=lb_simp_rec.get("simplex_index") if lb_simp_rec else None,
        )
        
        # Determine which source was chosen (use explicit vars, not dir() checks)

        chosen_source = "MESH_NODE"
        library_winner = None
        if UB_node != tuple(nodes[mesh_ub_node_idx]) if isinstance(mesh_ub_node_idx, int) else True:
            if ef_accepted:
                chosen_source = "EF"
            elif avg_cs_accepted:
                chosen_source = "AVG_CS"
            else:
                # Check if from library
                chosen_source = "LIBRARY"
                # Find which library entry matches UB_node
                for lib_idx, (lib_pt, lib_val) in enumerate(ub_candidate_library):
                    if lib_pt == UB_node:
                        library_winner = {"index": lib_idx, "x": list(lib_pt), "ub_value": lib_val}
                        break
        
        # Log the iteration record
        ub_logger.log_iter({
            "iter": it,
            "prev_ub": float(UB_hist[-2]) if len(UB_hist) >= 2 else None,
            "candidates": ub_candidates,
            "chosen": {"source": chosen_source, "ub_value": float(UB_global)},
            "final_ub": float(UB_global),
            "best_ub_ever": float(best_ub_ever),
            "best_lb_ever": float(best_lb_ever),
            "ub_node": list(UB_node) if UB_node else None,
            "library_winner": library_winner,  # Details if UB came from library
    ...
    return {
        ...
        "ub_log_dir": str(ub_logger.run_dir),  # Include log directory in return
