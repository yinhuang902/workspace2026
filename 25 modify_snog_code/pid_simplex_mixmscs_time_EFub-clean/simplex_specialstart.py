...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    _t_phase = perf_counter()
    ef_ub = None
    ef_log_path = "simplex_ef_info.txt"
    bound_debug_path = "simplex_bound_debug.txt"
    if enable_ef_ub:
        try:
            ef_ub = SimplexEFUb(
            ...
            if verbose:
                print(f"[EF-UB] Initialized SimplexEFUb with {S} scenarios, "
                      f"time_ub={ef_time_ub}s")
        except Exception as e:
            print(f"[EF-UB] WARNING: Failed to build EF model: {e}")
            ef_ub = None
            enable_ef_ub = False
    ...
    while len(nodes) < target_nodes:
        ...
        if enable_ef_ub and ef_ub is not None:
            ef_iter_info["ef_attempted"] = True
            try:
                # Extract the 4 vertices of the selected simplex
                verts4 = [tuple(map(float, v)) for v in lb_simp_rec["verts"]]
                ef_ub.update_simplex_vertices(verts4)

                # Warm-start EF from per-scenario c_s solutions (analogous
                # to SNoGloDe loading Gurobi incumbents into shared variables)
                c_pts = lb_simp_rec.get("c_point_per_scene", [])
                if c_pts:
                    ef_ub.set_warm_start(c_pts)

                ef_ok, K_ef, ef_obj, ef_solve_info = ef_ub.solve()

                ef_iter_info["ef_ok"] = ef_ok
                ef_iter_info["solver_status"] = ef_solve_info.get("solver_status")
                ef_iter_info["termination_condition"] = ef_solve_info.get("termination_condition")
                ef_iter_info["ef_time_sec"] = ef_solve_info.get("time_sec", 0.0)
                ef_iter_info["K_ef"] = K_ef
                ef_iter_info["ef_obj"] = ef_obj

                if ef_ok and K_ef is not None:
                    # Evaluate the TRUE expected objective (sum over scenarios)
                    true_obj = evaluate_true_expected_obj(
                        base_bundles, first_vars_list, K_ef
                    )
                    ef_iter_info["true_obj"] = true_obj

                    # Update UB incumbent if this improves it
                    if true_obj < UB_incumbent:
                        UB_incumbent = true_obj
                        UB_global = UB_incumbent
                        UB_node = K_ef
                        ub_updated_this_iter = True
                        ub_source_this_iter = "EF_selected_simplex"
                        ub_simplex_id_this_iter = tuple(sorted(lb_simp_rec["vert_idx"]))
                        ef_iter_info["ub_updated_by_ef"] = True
                        if verbose:
                            print(f"[Iter {it}] EF-UB improved incumbent: "
                                  f"{true_obj/S:.6e} (per-scen), K={K_ef}")
                    else:
                        if verbose:
                            print(f"[Iter {it}] EF-UB solved OK but no improvement: "
                                  f"true_obj={true_obj/S:.6e} >= UB={UB_incumbent/S:.6e}")
                else:
                    if verbose:
                        print(f"[Iter {it}] EF-UB solve failed: "
                              f"status={ef_iter_info['solver_status']}, "
                              f"term={ef_iter_info['termination_condition']}")

            except Exception as e:
                ef_iter_info["solver_status"] = "exception"
                ef_iter_info["termination_condition"] = str(e)
                if verbose:
                    print(f"[Iter {it}] EF-UB exception: {e}")
        ...
        # === Per-scenario bound debug (write to simplex_bound_debug.txt) ===
        try:
            ...
            for s in range(S):
                ...
                if (ms_lb is not None and ms_pri is not None
                    ...
                    ms_lb_gt_count += 1
                ms_entries[key] = {
                    "term": mm.get("termination_condition", ""),
                    "status": mm.get("status", ""),
                    "lb": ms_lb,
                    "primal": ms_pri,
                    "lb_gt_primal": ms_lb_gt,
                    "fallback": mm.get("fallback_reason"),
                }
                # CS
                ...
                if (cs_lb is not None and cs_pri is not None
                    ...
                    cs_lb_gt_count += 1
                cs_entries[key] = {
                    "term": cm.get("termination_condition", ""),
                    "status": cm.get("status", ""),
                    "lb": cs_lb,
                    "primal": cs_pri,
                    "lb_gt_primal": cs_lb_gt,
                    "fallback": cm.get("fallback_reason"),
                }
            debug_rec = {
                "iter": it,
                "simplex_id": list(sorted(lb_simp_rec["vert_idx"])),
                "ms_lb_gt_primal_count": ms_lb_gt_count,
                "cs_lb_gt_primal_count": cs_lb_gt_count,
                "ms": ms_entries,
                "cs": cs_entries,
            }
            with open(bound_debug_path, "a", encoding="utf-8") as fdbg:
                fdbg.write(json.dumps(debug_rec, default=str) + "\n")
            # Inject counts into deferred log row
            if _ef_log_row is not None:
                _ef_log_row["ms_lb_gt_primal_count"] = ms_lb_gt_count
                _ef_log_row["cs_lb_gt_primal_count"] = cs_lb_gt_count
        except Exception:
        ...
        if selected_rec_before_split is not None:
            ...
            _diag_lines.append(f"  Parent simplex: {parent_id}")
            _diag_lines.append(f"  Parent LB:      {parent_LB/S:.9f} (per-scen)")
            _diag_lines.append(f"  Prev global LB: {prev_global_min_LB/S:.9f} (per-scen, min over all pre-split)")
            _diag_lines.append(f"  New children ({n_children}):")
            ...
            for new_idx in new_simplex_indices:
                if new_idx in per_tet_dict:
                    ...
                    marker = ""
                    if child_lb < parent_LB - 1e-10:
                        marker = " *** CHILD LB < PARENT LB ***"
                    if child_lb < prev_global_min_LB - 1e-10:
                        marker += " *** CHILD LB < PREV GLOBAL LB ***"
                    _diag_lines.append(f"    T{new_idx} {child_id}: LB = {child_lb/S:.9f}{marker}")
            
            # Also show the non-split simplices that might have the global min
            old_simplex_indices = [r["simplex_index"] for r in per_tet_end 
                                   if r["simplex_index"] not in new_simplex_indices]
            old_min_rec = min((r for r in per_tet_end if r["simplex_index"] in old_simplex_indices),
                             key=lambda r: r["LB"], default=None)
            if old_min_rec:
                old_min_lb = float(old_min_rec["LB"])
                old_min_id = tuple(sorted(old_min_rec["vert_idx"]))
                _diag_lines.append(f"  Min LB among OLD (unchanged) simplices: T{old_min_rec['simplex_index']} {old_min_id}: {old_min_lb/S:.9f}")
            
            new_global_min_id = tuple(sorted(lb_simp_rec_end["vert_idx"]))
            _diag_lines.append(f"  New global LB:  {LB_global_end/S:.9f} (per-scen, from T{lb_simp_rec_end['simplex_index']} {new_global_min_id})")
            
            lb_decreased = LB_global_end < prev_global_min_LB - 1e-10
            if lb_decreased:
                _diag_lines.append(f"  >> LB DECREASED: {prev_global_min_LB/S:.9f} -> {LB_global_end/S:.9f} "
                      f"(delta = {(LB_global_end - prev_global_min_LB)/S:.2e})")
                if lb_simp_rec_end["simplex_index"] in new_simplex_indices:
                    _diag_lines.append(f"  >> CAUSE: A child simplex has lower LB than any pre-split simplex")
                else:
                    _diag_lines.append(f"  >> CAUSE: An unchanged simplex has lower LB (possibly due to cache/re-eval)")
            else:
                _diag_lines.append(f"  >> LB did not decrease (OK)")
            _diag_lines.append("=" * 70)
            ...
            # Write to file (append mode, flushed immediately)
            with open("simplex_lb_split_diag.txt", "a", encoding="utf-8") as _fdiag:
                _fdiag.write("\n".join(_diag_lines) + "\n\n")
        ...
        # === Write deferred EF/MS log row (now LB_global_end and UB_global_end are final) ===
        try:
            if _ef_log_row is not None:
                _ef_log_row["LB_global_end_sum"] = f"{LB_global_end:.6f}"
                _ef_log_row["UB_global_end_sum"] = f"{UB_global_end:.6f}"
                append_ef_info(ef_log_path, _ef_log_row)
        except Exception:
