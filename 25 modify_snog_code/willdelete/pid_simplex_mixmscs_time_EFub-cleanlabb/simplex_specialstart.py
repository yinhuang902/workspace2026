            ...
            if hit:
                ...
                if _ms_meta and _ms_meta.get("status") == "time_limit":
                    ...
                    if _tl_dual is not None and _tl_prim is not None:
                        ...
                        _dual_lt_prim = None
                    _tl_line = (
                        f"[MS TimeLimit] simplex_idx={k}, verts={simplex_id}, "
                ...
                if _cs_meta and _cs_meta.get("status") == "time_limit":
                    ...
                    if _tl_dual is not None and _tl_prim is not None:
                        ...
                        _dual_lt_prim = None
                    _tl_line = (
                        f"[CS TimeLimit] simplex_idx={k}, verts={simplex_id}, "
                        f"scenario={ω}, dual_bound={_tl_dual}, primal_obj={_tl_prim}, "
                        f"dual<primal={_dual_lt_prim}\n"
                    )
                    try:
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    # === ACTIVE-SET FILTER ===
    # NOTE: We do NOT delete inactive simplexes (no del/pop/clear) because that
    # would break ms_cache keys, lb_sur_cache keys, tet_mesh.tets indices, and
    # tracker references.  Instead we keep an active-set and filter everywhere.
    # Filtering is strictly safer: inactive simplexes are simply invisible to
    # selection, splitting, and all LB/UB aggregation.
    active_simplex_ids = None   # None = unrestricted (all active); set after iter 0

    def _sid_is_active(sid):
        """Return True if simplex index *sid* belongs to the active set."""
        return active_simplex_ids is None or sid in active_simplex_ids

    def iter_active_simplexes(records):
        """Yield per_tet records whose simplex_index is in the active set."""
        for r in records:
            if _sid_is_active(r["simplex_index"]):
                yield r

    def active_len():
        """Number of active simplexes (None means unrestricted)."""
        return len(active_simplex_ids) if active_simplex_ids is not None else None
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Time (s)", "# Nodes", "LB", "UB", "Rel. Gap", "Abs. Gap", "best_LB_ever", "best_UB_ever", "active_count", "is_active"])
    ...
    while len(nodes) < target_nodes:
        ...
        # === ACTIVE-SET: after the first evaluate_all_tetra (iter 0), lock to T1 ===
        if it == 0 and active_simplex_ids is None:
            # T1 is the simplex printed as "T1" in verbose output (simplex_index == 1).
            # We store its simplex_index; children produced by splitting T1 will be
            # added to active_simplex_ids in the subdivide section below.
            T1_ID = 1
            active_simplex_ids = {T1_ID}
            if verbose:
                print(f"[ActiveSet] Initialized: keeping only T{T1_ID} active "
                      f"(total initial simplexes = {len(per_tet)})")
        ...
        _t_phase = perf_counter()
        # Active mask now also respects the active-set filter so that inactive
        # simplexes are invisible to all downstream algorithmic decisions.
        active_mask = {
            r["simplex_index"]: (r["LB"] <= UB_global + active_tol) and _sid_is_active(r["simplex_index"])
            for r in per_tet
        }
        q_cut = 0
        ...
        if chosen_cand is not None:
            ...
            # Safety: never split an inactive simplex
            assert _sid_is_active(sid), f"Splitting inactive simplex T{sid}!"
        ...
        # === ACTIVE-SET: inherit children into active set ===
        # If the split parent was active, its children must also be active so the
        # algorithm can continue refining within the T1 subtree.
        if chosen_cand is not None and active_simplex_ids is not None:
            _parent_sid = int(chosen_cand["simplex_index"])
            if _sid_is_active(_parent_sid):
                n_simplices_after_split = len(tet_mesh.tets)
                for _new_idx in range(n_simplices_after_split - n_children, n_simplices_after_split):
                    active_simplex_ids.add(_new_idx)
        ...
        # LB_global_end = min over ACTIVE simplices of LB_local
        _active_per_tet_end = [r for r in per_tet_end if _sid_is_active(r["simplex_index"])]
        if _active_per_tet_end:
            LB_global_end = float(min(r["LB"] for r in _active_per_tet_end))
            lb_simp_rec_end = min(_active_per_tet_end, key=lambda r: r["LB"])
        else:
            # Fallback (should not happen if T1 subtree is always active)
        ...
        if selected_rec_before_split is not None:
            ...
            _ef_val = ef_iter_info.get("true_obj")
            if _ef_val is not None:
                _diag_lines.append(f"  EF true_obj: {_ef_val/S:.9f} (per-scen)")
            else:
                _diag_lines.append(f"  EF true_obj: N/A")
        ...
        for new_idx in new_simplex_indices:
            ...
            # (b) EF candidate: include the best EF solution as an additional UB candidate
            #     ef_iter_info["true_obj"] is a SUM over scenarios, same as UB_global_end
        if ef_iter_info.get("ef_ok") and ef_iter_info.get("true_obj") is not None:
            _ef_true = ef_iter_info["true_obj"]
            if math.isfinite(_ef_true) and _ef_true < UB_global_end:
                UB_global_end = _ef_true
                UB_node = ef_iter_info["K_ef"]
                ub_updated_end = True
                ub_source_end = "EF"
                ub_simplex_id_this_iter = tuple(sorted(lb_simp_rec["vert_idx"]))
                if verbose:
                    print(f"[Iter {it}] UB_global_end improved by EF: {_ef_true/S:.6e} (per-scen)")
        ...
        # Active-set CSV columns
        _csv_active_cnt = active_len() if active_len() is not None else len(per_tet_end)
        _csv_is_active = _sid_is_active(sid) if chosen_cand is not None else True
        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([f"{iter_time:.3f}", n_nodes, f"{lb_val:.9f}", f"{ub_val:.9f}", f"{rel_gap*100:.7f}%", f"{abs_gap:.5f}", f"{lb_ever:.9f}", f"{ub_ever:.9f}", _csv_active_cnt, _csv_is_active])
    ...
    if verbose and LB_hist:
        ...
        for k in range(len(LB_hist)):
            ...
            # Active-set count for this iteration (from active_simplex_ids)
            _t1act = active_len() if active_len() is not None else nact
