...
def solve_surrogate_lb_for_tet(tet_vertices, quad_coeffs_per_scene, ms_scene,
                                c_scene, quad_interp_ok, ms_bound_ok,
                                fverts_per_scene):
    """
    Compute surrogate LB for one simplex using the quadratic SurrogateLBBundle.

    Parameters
    ----------
    tet_vertices : list of vertex coordinates, shape (d+1, d)
    quad_coeffs_per_scene : list of (c_qs, g, H) or None, length S
    ms_scene : list of float (+inf sentinel for unavailable), length S
    c_scene  : list of float (-inf sentinel for unavailable), length S
    quad_interp_ok : list of bool, length S — interpolation succeeded?
    ms_bound_ok    : list of bool, length S — ms bound available?
    fverts_per_scene : list of list of float (vertex Q vals for fallback)
    """
    global _LB_BUNDLE

    S = len(c_scene)
    n_verts = len(fverts_per_scene[0]) if fverts_per_scene and fverts_per_scene[0] else len(tet_vertices)

    # Gate: quadratic constraint fires only when BOTH quad_interp_ok and ms_bound_ok
    quad_and_ms_ok = [qi and mi for qi, mi in zip(quad_interp_ok, ms_bound_ok)]

    if (_LB_BUNDLE is None) or (_LB_BUNDLE.S != S) or (_LB_BUNDLE._n_verts != n_verts):
        _LB_BUNDLE = SurrogateLBBundle(S, n_verts=n_verts)

    LB_sur = _LB_BUNDLE.compute_lb(
        tet_vertices=tet_vertices,
        quad_coeffs_per_scene=quad_coeffs_per_scene,
        ms_scene=ms_scene,
        c_scene=c_scene,
        quad_available=quad_and_ms_ok,
...
def ms_on_tetra_for_scene(ms_bundle, tet_vertices, fverts_scene,
                          base_bundle, first_vars, d,
                          quad_logger=None, iter_num=None,
                          simplex_id=None, scenario_id=None):
    """
    Solve c_s FIRST, then build quadratic interpolation, then solve ms.

    Flow:
        1. solve_const_cut → get cs_point, cs_qval
        2. build_interpolation_set (cs_point → vertices → det → random)
        3. If success: update_tetra(verts, quad_coeffs), solve ms
        4. If failure: mark quad_unavailable, skip ms

    Returns
    -------
    dict with keys:
        ms_val       : float — ms dual bound (+inf if unavailable)
        new_pt_ms    : tuple or None — ms primal point (None if unavailable)
        c_val        : float — c_s dual bound (-inf if unavailable)
        c_pt         : tuple or None — c_s primal point
        quad_coeffs  : (c_qs, g_vec, H_mat) or None
        quad_status  : dict with quad_interp_ok, ms_bound_ok, ms_point_ok,
                       cs_bound_ok, cs_point_ok, skip_reason
    """
    from quadratic_surrogate import build_interpolation_set, quad_basis_size

    result = {
        "ms_val": float('+inf'),     # +inf sentinel: unavailable
        "new_pt_ms": None,
        "c_val": float('-inf'),      # -inf sentinel: unavailable
        "c_pt": None,
        "quad_coeffs": None,
        "quad_status": {
            "quad_interp_ok": False,
            "ms_bound_ok": False,
            "ms_point_ok": False,
            "cs_bound_ok": False,
            "cs_point_ok": False,
    ...
    if cs_result["bound_ok"]:
        ...
        cs_point_use = cs_result["c_pt"] if cs_result["point_ok"] else None
        cs_qval_use = cs_result["c_val"]
    else:
        print(f"[ms_on_tetra] scen {scenario_id}: c_s solve failed")
        cs_point_use = None
        cs_qval_use = None

    if cs_result["point_ok"]:
        result["c_pt"] = cs_result["c_pt"]
    ...
    if not interp_result["success"]:
        result["quad_status"]["skip_reason"] = interp_result["reason"]
        # Log if logger available
        if quad_logger is not None:
            diag = interp_result["diag"]
            quad_logger.log_scenario(
                iter_num=iter_num or 0, simplex_id=simplex_id,
                scenario_id=scenario_id or 0,
                cs_available=diag.get("cs_available", False),
                cs_used=diag.get("cs_used", False),
                vertices_used=diag.get("vertices_used", []),
                vertices_discarded=diag.get("vertices_discarded", []),
                det_tried=diag.get("det_candidates_tried", 0),
                det_used=diag.get("det_candidates_used", 0),
                random_attempts=diag.get("random_attempts", 0),
                random_used=diag.get("random_used", 0),
                final_points=interp_result.get("points"),
                rank=interp_result.get("rank", 0),
                cond=interp_result.get("cond", float('inf')),
                quad_available=False,
                ms_solved=False,
                skip_reason=interp_result["reason"],
                contributed_via="c_s_only",
                surrogate_lb_included="c_s_only",
            )
        return result

    # Quadratic interpolation succeeded
    quad_coeffs = interp_result["quad_coeffs"]
    result["quad_coeffs"] = quad_coeffs
    ...
    if solve_result["bound_ok"]:
        ms_val, lam_star, new_pt_ms = ms_bundle.get_ms_and_point()
        if math.isfinite(ms_val):
            result["ms_val"] = ms_val
        # else: ms_val is +inf, stays as sentinel
        if solve_result["point_ok"] and new_pt_ms is not None:
            result["new_pt_ms"] = new_pt_ms
    else:
        result["quad_status"]["skip_reason"] = "ms_solve_failure"
    ...
    if quad_logger is not None:
        ...
        quad_logger.log_scenario(
            ...
            cond=interp_result.get("cond", float('inf')),
            quad_available=True,
            ms_solved=result["quad_status"]["ms_bound_ok"],
            skip_reason=result["quad_status"]["skip_reason"],
            contributed_via="q_s_plus_ms" if result["quad_status"]["ms_bound_ok"] else "c_s_only",
            surrogate_lb_included="quadratic" if result["quad_status"]["ms_bound_ok"] else "c_s_only",
        )
...
def evaluate_all_tetra(nodes, scen_values, ms_bundles, first_vars_list,
                       ...
                       quad_logger=None):
    """
    Evaluate all simplices with quadratic surrogates.

    Parameters added for quadratic:
        base_bundles : list[BaseBundle] — for Q_s evaluation at interior points
        quad_logger  : QuadInterpLogger — for per-scenario diagnostics
    """
    ...
    for k, simp in enumerate(simplices):
        ...
        # ==========  per-scene quadratic ms + constant-cut  ==========
        key_base = tuple(sorted(idxs))
        ms_scene = []               # float (+inf if unavailable)
        xms_scene = []              # tuple or None
        c_scene = []                # float (-inf if unavailable)
        cpts_scene = []             # tuple or None
        quad_coeffs_scene = []      # (c,g,H) or None
        quad_status_scene = []      # list of status dicts
        quad_interp_ok_scene = []   # bool per scene
        ms_bound_ok_scene = []      # bool per scene
        ms_point_ok_scene = []      # bool per scene
        ms_meta_per_scene = []
        cs_meta_per_scene = []
        ...
        for w in range(S):
            ...
            if hit:
                cached = ms_cache[cache_key]
                ms_scene.append(cached["ms_val"])
                xms_scene.append(cached["new_pt_ms"])
                c_scene.append(cached["c_val"])
                cpts_scene.append(cached["c_pt"])
                quad_coeffs_scene.append(cached["quad_coeffs"])
                quad_status_scene.append(cached["quad_status"])
                qs = cached["quad_status"]
                quad_interp_ok_scene.append(qs.get("quad_interp_ok", False))
                ms_bound_ok_scene.append(qs.get("ms_bound_ok", False))
                ms_point_ok_scene.append(qs.get("ms_point_ok", False))
                ...
                scene_result = ms_on_tetra_for_scene(
                    ...
                    tet_vertices=verts,
                    fverts_scene=fverts_per_scene[w],
                    base_bundle=base_bundles[w] if base_bundles else None,
                    first_vars=first_vars_list[w] if first_vars_list else None,
                    d=d,
                ...
                c_scene.append(scene_result["c_val"])
                cpts_scene.append(scene_result["c_pt"])
                quad_coeffs_scene.append(scene_result["quad_coeffs"])
                quad_status_scene.append(scene_result["quad_status"])
                qs = scene_result["quad_status"]
                quad_interp_ok_scene.append(qs.get("quad_interp_ok", False))
                ms_bound_ok_scene.append(qs.get("ms_bound_ok", False))
                ms_point_ok_scene.append(qs.get("ms_point_ok", False))

                # Capture solve metadata
                ...
                if _debug_buf is not None:
                    ms_val_dbg = scene_result["ms_val"]
                    c_val_dbg = scene_result["c_val"]
                    for _tag, _meta, _val in [("ms", _ms_meta, ms_val_dbg), ("cs", _cs_meta, c_val_dbg)]:
                        if _meta is None:
                            continue
                        _st = _meta.get("status", "")
                ...
                if cache_on and (ms_cache is not None):
                    ms_cache[cache_key] = scene_result
        ...
        # ---- Aggregation ----
        # ms_total: always a float (sum of per-scene ms_val; +inf if any unavailable)
        ms_total = float(sum(ms_scene))  # +inf if any scene has +inf

        # ms_diag: diagnostic sum of scenes with valid bound (may be None)
        avail_ms = [(s, ms_scene[s]) for s in range(S) if ms_bound_ok_scene[s]]
        ms_diag = float(sum(v for _, v in avail_ms)) if avail_ms else None

        # best_scene: scene with min ms among those with bound + point
        avail_pts = [
            (s, ms_scene[s]) for s in range(S)
            if ms_bound_ok_scene[s] and ms_point_ok_scene[s]
        ]
        best_scene = min(avail_pts, key=lambda t: t[1])[0] if avail_pts else None
        x_ms_best = xms_scene[best_scene] if best_scene is not None else None

        c_arr = np.asarray(c_scene, float)  # c_scene already uses -inf sentinel
        finite_c_mask = np.isfinite(c_arr)
        c_total = float(np.sum(c_arr[finite_c_mask])) if np.any(finite_c_mask) else float('-inf')

        # UB: use vertex sums + ms_total (naturally +inf when ms unavailable)
        UB = float(np.max(fverts_sum) + ms_total)

        # Surrogate LB via quadratic SurrogateLBBundle
        if lb_sur_cache is not None and key_base in lb_sur_cache:
            LB_sur = lb_sur_cache[key_base]
        else:
            LB_sur = solve_surrogate_lb_for_tet(
                tet_vertices=verts,
                quad_coeffs_per_scene=quad_coeffs_scene,
                ms_scene=ms_scene,
                c_scene=c_scene,
                quad_interp_ok=quad_interp_ok_scene,
                ms_bound_ok=ms_bound_ok_scene,
                fverts_per_scene=fverts_per_scene,
            )
        ...
        # Quad summary for logging
        n_quad_interp_ok = sum(quad_interp_ok_scene)
        n_ms_bound_ok = sum(ms_bound_ok_scene)
        n_ms_point_ok = sum(ms_point_ok_scene)
        if quad_logger is not None:
            simplex_status = "usable" if n_ms_bound_ok > 0 or np.any(finite_c_mask) else "unusable_all_fail"
            quad_logger.log_simplex_summary(
                iter_num=iter_num or 0, simplex_id=simplex_id,
                n_quad_available=n_quad_interp_ok,
                n_quad_unavailable=S - n_quad_interp_ok,
                n_ms_solved=n_ms_bound_ok,
                n_cs_only=S - n_ms_bound_ok,
                simplex_status=simplex_status,
                lb_value=LB_sur,
            )

        # Compute LB_terms diagnostic summary for downstream logging
        _min_fverts_sum = float(np.min(fverts_sum))
        if math.isfinite(LB_sur) and math.isfinite(ms_total) and math.isfinite(c_total):
            lb_case = "surrogate_qcp"
        elif math.isfinite(c_total):
            lb_case = "c_only"
        else:
            lb_case = "fallback"
        ...
        per_tet.append({
            ...
            "quad_coeffs_per_scene": quad_coeffs_scene,
            "quad_status_per_scene": quad_status_scene,
            "quad_interp_ok": quad_interp_ok_scene,
            "ms_bound_ok": ms_bound_ok_scene,
            "ms_point_ok": ms_point_ok_scene,
            "ms": ms_total,
            "ms_diag": ms_diag,
            "c_agg": c_total,
            "LB": LB_sur,
            ...
            "LB_terms": {
                "LB_linear": LB_sur,
                "c_total_finite": c_total if math.isfinite(c_total) else float('nan'),
                "c_total_all": c_total,
                "ms_total": ms_total,
                "min_fverts_sum": _min_fverts_sum,
                "lb_case": lb_case,
            },
        })
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    ms_cache = {}   # (scene_idx, sorted(vert_idx)) -> scene_result dict
                     # Keys: ms_val (+inf sentinel), new_pt_ms, c_val (-inf sentinel),
                     # c_pt, quad_coeffs, quad_status {quad_interp_ok, ms_bound_ok,
                     # ms_point_ok, cs_bound_ok, cs_point_ok, skip_reason}
    ...
    # File 10: Quadratic interpolation diagnostics
    from quadratic_surrogate import QuadInterpLogger
    _quad_logger = QuadInterpLogger(os.path.join(debug_log_dir, "debug_quad_interp.txt"))
    ...
    while len(nodes) < target_nodes:
        ...
        ms_b      = float(lb_simp_rec["ms"])         # mathematical: may be +inf
        ms_b_diag = lb_simp_rec.get("ms_diag")         # diagnostic: may be None
        ...
        if any(active_mask.values()):
            _ms_vals = [r["ms"] for r in per_tet if active_mask[r["simplex_index"]] and math.isfinite(r["ms"])]
            ms_a = float(min(_ms_vals)) if _ms_vals else float('nan')
        else:
            _ms_vals = [r["ms"] for r in per_tet if math.isfinite(r["ms"])]
            ms_a = float(min(_ms_vals)) if _ms_vals else float('nan')
        ms_iter = ms_a
        ...
        if verbose:
            ...
            _ms_b_str = f"{ms_b_diag:.3e}" if ms_b_diag is not None else "N/A"
            print(f"[Iter {it}] LB = {LB_global:.6f} = UB({UB_global:.6f}) + ms_b({_ms_b_str}) from {msb_src}")
        ...
        for rec in pool_records:
            ...
            if use_c_fallback:
                ...
                # Always add BOTH ms and c_s as separate candidates (if available)
                for s in range(len(ms_vals)):
                    ms_pt = ms_pts[s]
                    # Add ms candidate only if ms is available
                    if ms_vals[s] is not None and ms_pt is not None:
                        cand_items.append({
                            "simplex_index": sid,
                            "scene": s,
                            "cand_ms": ms_vals[s],
                    ...
                    # Add c_s candidate (if available)
                    c_pt = c_pts[s] if s < len(c_pts) else None
                    if c_pt is not None:
                        c_val = c_vals[s] if s < len(c_vals) else None
                ...
                "per_tet_snapshot": [
                    {
                        ...
                        "ms": float(r["ms"]) if r["ms"] is not None else None,
                        "ms_per_scene": [float(x) if x is not None else None for x in r.get("ms_per_scene", [])],
                        "LB": float(r["LB"]),
                        "UB": float(r["UB"]),
                        "best_scene": int(r["best_scene"]) if r["best_scene"] is not None else None,
        ...
        if POINT_SELECT_MODE == 2:
            ...
            for ms_val, pt in zip(ms_list, pts_list):
                if pt is None or not math.isfinite(ms_val):
                    continue
            ...
            if weights:
                ...
                if min_dist_to_nodes(candidate_pt, nodes) >= min_dist:
                    ...
                    ms_list_clean = [float(v) if math.isfinite(v) else 0.0 for v in ms_list]
                    chosen_ms  = float(np.dot(w_arr, np.array(ms_list_clean, float)))
                    chosen_cand = {
        ...
        if it == 0:
            try:
                ...
                for r in sorted(per_tet, key=lambda rr: rr["simplex_index"]):
                    ...
                    # Avg constant-cut bound = average of c_s across scenarios
                    ...
                    _i0_lines.append(f"    Avg constant-cut c_s: {_avg_dual_str}")
                ...
                if _is_mode2or3:
                    ...
                    if lambdas is not None:
                        ...
                        for _s in range(S):
                            ...
                            qs_val = float(np.dot(lambdas, q_verts))
                            _ms_raw = rec["ms_per_scene"][_s]
                            ms_val = float(_ms_raw) if math.isfinite(_ms_raw) else float('nan')
                            qs_plus_ms = qs_val + ms_val
                            q_val = float(new_vals[_s])
                            _ms_str = f"{ms_val:.4e}" if math.isfinite(ms_val) else "N/A"
                            _qm_str = f"{qs_plus_ms:.4e}" if math.isfinite(qs_plus_ms) else "N/A"
                            print(fmt_row([_s, f"{qs_val:.4e}", _ms_str,
                                           _qm_str, f"{q_val:.4e}"]))
                        print()
                else:
                    ...
                    if lambdas is not None:
                        ...
                        qs_val = float(np.dot(lambdas, q_verts))
                        _ms_raw = rec["ms_per_scene"][scene]
                        ms_val = float(_ms_raw) if math.isfinite(_ms_raw) else float('nan')
                        qs_plus_ms = qs_val + ms_val
                        q_val = float(new_vals[scene])

                        coord_header = "(" + ", ".join(f"x{i+1}" for i in range(len(new_node))) + ")"
                        header = ["Simplex", "Scene", "Type", "qs", "ms", "qs+ms", "Q", coord_header]
                        ...
                        row = [
                            ...
                            f"{qs_val:.4e}", _ms_str, _qm_str, f"{q_val:.4e}",
                            coords_str
                        ]
                        print(fmt_row(row))
                        print()
