...
class Subproblems():
    ...
    def solve_quad_shift_subproblem(self, 
                                    subproblem_name: str, 
                                    q_surrogate, 
                                    lower_bounder) -> float:
        """
        Solves min(Obj - Q) for a given subproblem.
        """
        from snoglode.utils.supported import SupportedVars
        import pyomo.environ as pyo
        ...
        # Get Reals in Order
        lifted_vars = self.subproblem_lifted_vars[subproblem_name]
        
        # Map ID -> Var
        reals_map = {}
        for v in lifted_vars:
            # Check correctness of var_to_data mapping
            if v in self.var_to_data:
                domain, vid, _ = self.var_to_data[v]
                if domain == SupportedVars.reals:
                    reals_map[vid] = v
        
        # Sort by ID to match Q-surrogate feature order
        sorted_ids = sorted(reals_map.keys())
        sorted_vars = [reals_map[vid] for vid in sorted_ids]
        
        # Verify dimension
        if len(sorted_vars) != q_surrogate.dim:
            # Mismatch. Maybe some vars are not in this subproblem?
            # For now, return -inf or 0 if we can't solve properly?
            # Or handle graceful failure?
            # This method computes a shift.
            # If we miss vars, Q(x) is incomplete.
            # Let's assume standard case for now.
            pass

        # 1. Save original objective
        # Find objective component
        obj_component = None
        for obj in model.component_objects(pyo.Objective, active=True):
            obj_component = obj
            break
            
        if obj_component is None:
            return 0.0 # No objective?
            
        original_expr = obj_component.expr
        
        # 2. Build Q expression
        # Q(x)
        try:
            q_expr = q_surrogate.build_pyomo_expression(sorted_vars)
        except Exception as e:
            print(f"Error building Q expr: {e}")
            return 0.0

        # New Obj: F - Q
        # Assuming Min F
        obj_component.expr = original_expr - q_expr
        
        # 3. Solve
        # Use lower_bounder solver if available
        opt = None
        if hasattr(lower_bounder, 'opt'):
            opt = lower_bounder.opt
        elif hasattr(lower_bounder, 'solver') and not isinstance(lower_bounder.solver, str):
            opt = lower_bounder.solver
        else:
        ...
        try:
            ...
            if res.solver.termination_condition == TerminationCondition.infeasible:
                ...
                val = pyo.value(obj_component.expr)
        except Exception as e:
            print(f"Error solved quad shift: {e}")
            val = float('inf') # Fail safe
            
        # 4. Restore
        obj_component.expr = original_expr
        
        return val

    def compute_global_shift(self,
                             q_surrogate,
                             lower_bounder) -> float:
        """
        Computes the global shift m = sum p_s * m_s, where m_s = min (F_s - Q)
        over the GLOBAL domain (root node bounds).
        
        This involves:
        1. Saving current bounds.
        2. Setting bounds to root node bounds.
        3. Solving modified subproblems.
        4. Restoring bounds.
        5. Aggregating m.
        """
        # 1. Save current bounds & Set to root
        # We can iterate through self.vars.reals if available or rely on self.subproblem_lifted_vars
        # But we need to do this for ALL subproblems.
        
        # It's cleaner to use set_all_states logic but targeting root state.
        # But we need to save the current state first?
        # Actually, the solver loop controls state.
        # This method is called during UB solve phase (refitting), 
        # where the node state is typically set to the current node.
        # So yes, we must save and restore.
        
        # Optimization: We only need to change bounds of lifted reals.
        # Let's verify if we have access to variables easily.
        
        # We can store current bounds in a dict: {sub_name: {var_name: (lb, ub)}}
        saved_bounds = {}
        
        from snoglode.utils.supported import SupportedVars
        if SupportedVars.reals not in self.root_node_state:
            return 0.0 # No lifted reals?
            
        root_reals = self.root_node_state[SupportedVars.reals] # {id: bounds}
        
        for sub_name in self.model:
            saved_bounds[sub_name] = {}
            for v in self.subproblem_lifted_vars[sub_name]:
                # Identify ID
                _, vid, _ = self.var_to_data[v]
                # Save using name/ID which is hashable
                # vid is the string ID
                saved_bounds[sub_name][vid] = (v.lb, v.ub, v) # Store var obj reference in value
                
                # Set to Root
                r_lb = root_reals[vid].lb
                r_ub = root_reals[vid].ub
                v.setlb(r_lb)
                v.setub(r_ub)
        ...
        for sub_name in self.names:
            prob = self.probability[sub_name]
            # Re-use solve_quad_shift_subproblem since it solves min (F-Q)
            # and uses current bounds (which are now global).
            val = self.solve_quad_shift_subproblem(sub_name, q_surrogate, lower_bounder)
            
            if val == float('inf'):
                # If infeasible globally? Should not happen if root is feasible.
                # But if it does, m = inf? No, we need valid underestimator.
                # If F is infeasible, F >= Q + m holds for m = -inf?
                # Actually if F is inf, min(F-Q) is inf.
                # But we want m s.t. F >= Q + m -> m <= F - Q
                # So if F=inf, m can be anything.
                # But we aggregate. If one scenario infeasible, m -> inf?
                # Let's cap at large value or keep inf?
                # Usually we want a finite lower bound.
                # If F is genuinely infeasible globally, then LB is inf.
                m_total_local += prob * 1e8 # Cap? Or inf.
            else:
                m_total_local += prob * val
                
        # 3. Restore bounds
        for sub_name in self.model:
            for vid, (lb, ub, v) in saved_bounds[sub_name].items():
                v.setlb(lb)
                v.setub(ub)
                
        return m_total_local
