...
def evaluate_all_tetra(nodes, scen_values, ms_bundles, first_vars_list,
                       ...
                       base_bundles=None, edge_cache=None):
    """
    Evaluate all Delaunay simplex formed by the node set
    across all scenarios, computing their ms values (Linear & Quadratic),
    ...
    For each simplex(tetrahedron):
        ...
        - Calculates/Retrieves edge midpoint values.
        - Solves Linear MS and Quadratic MS. Picks best.
        - Aggregates per-scene ms values.
    ...
    # Dictionary for edge indexing: (0,1), (0,2)...
    # Maps local index 0..5 to pairs of local vertex indices
    ...
    for k, simp in enumerate(simplices):
        ...
        # idxs is a list of global node indices
        ...
        # --- Edge Midpoints Logic ---
        # f_edges_per_scene[s][edge_k]
        f_edges_per_scene = [[] for _ in range(S)]
        
        # Only compute if we have bundles to evaluate
        if base_bundles and edge_cache is not None:
            # For each of the 6 edges
            current_mid_vals = [] # list of [val_s0, val_s1...] for each edge
            for (u_local, v_local) in local_edge_pairs:
                u_global, v_global = idxs[u_local], idxs[v_local]
                # Canonical edge key
                ekey = tuple(sorted((u_global, v_global)))
                
                if ekey in edge_cache:
                    vals = edge_cache[ekey]
                else:
                    # Compute midpoint and evaluate
                    mid_pt = (pts[u_global] + pts[v_global]) * 0.5
                    mid_pt_tuple = tuple(mid_pt)
                    vals = []
                    for s in range(S):
                        val = evaluate_Q_at(base_bundles[s], first_vars_list[s], mid_pt_tuple)
                        vals.append(val)
                    edge_cache[ekey] = vals
                
                current_mid_vals.append(vals)

            # Transpose to [scenario][edge]
            for edge_k in range(6):
                for s in range(S):
                    f_edges_per_scene[s].append(current_mid_vals[edge_k][s])
        else:
            # Fallback if no bundles passed (should not happen in main loop)
            f_edges_per_scene = [[0.0]*6 for _ in range(S)]
        ...
        tags_scene = []       # "ms(linear)" or "ms(quadra)"
        rec_ranges_q = []
        rec_ranges_q_ms = []
        ...
        for ω in range(S):
            ...
            # Cache structure: (ms_val, new_pt_ms, c_val, c_pt, tag)
            # Old cache might not have tag, handle that gracefully if needed
            if hit:
                cached_data = ms_cache[cache_key]
                if len(cached_data) == 7: # Updated cache structure
                    ms_final, new_pt_final, c_val, c_pt, tag, rng_q, rng_q_ms = cached_data
                elif len(cached_data) == 5: # Old cache structure with tag
                    ms_final, new_pt_final, c_val, c_pt, tag = cached_data
                    # Recompute ranges if not in cache
                    f_all_10 = list(fverts_per_scene[ω]) + list(f_edges_per_scene[ω])
                    min_q = min(f_all_10)
                    max_q = max(f_all_10)
                    rng_q = (min_q, max_q)
                    # Cannot determine ms_quad from cache, so approximate rng_q_ms or set to None
                    # For now, set to None, as we don't have ms_quad directly
                    rng_q_ms = (float('nan'), float('nan')) # Indicate not properly computed
                else: # Older cache structure without tag
                    ms_final, new_pt_final, c_val, c_pt = cached_data[:4]
                    tag = "ms(linear)" # default
                    # Recompute ranges if not in cache
                    f_all_10 = list(fverts_per_scene[ω]) + list(f_edges_per_scene[ω])
                    min_q = min(f_all_10)
                    max_q = max(f_all_10)
                ...
                bundle = ms_bundles[ω]
                # 1. Update Tetra geometry
                bundle.update_tetra(verts, fverts_per_scene[ω])

                # 2. Solve Linear MS
                # Explicitly call bundle.solve() which does linear
                ok_lin = bundle.solve()
                if ok_lin:
                    ms_lin, _, pt_lin = bundle.get_ms_and_point()
                else:
                    ms_lin = float('inf')
                    pt_lin = None
                
                # 3. Solve Quadratic MS
                edge_vals = f_edges_per_scene[ω]
                ok_quad = bundle.solve_quadratic(edge_vals)
                if ok_quad:
                    ms_quad, _, pt_quad = bundle.get_ms_and_point()
                else:
                    ms_quad = float('inf')
                    pt_quad = None
                
                # Calc range of As_quad (min/max of 10 control points)
                # It's a Bernstein polynomial, so it's contained in the convex hull of its control points.
                # So [min(pts), max(pts)] is a valid enclosing interval.
                f_all_10 = list(fverts_per_scene[ω]) + list(edge_vals)
                min_q = min(f_all_10)
                max_q = max(f_all_10)
                
                rng_q = (min_q, max_q)
                # range_as_quad_ms: shifted by ms_quad. Handle ms_quad being inf.
                if ok_quad: # ms_quad is finite
                    rng_q_ms = (min_q + ms_quad, max_q + ms_quad)
                else: # ms_quad is inf, so the shifted range is also inf
                    rng_q_ms = (float('inf'), float('inf'))

                # 4. Compare and Select
                # Prefer Quad if valid and smaller (better underestimator shift). 
                # Note: 'ms' here usually means (Qs - As), which is negative or small. 
                # We want the tightest lower bound? 
                # The Global LB is UB + ms. Since ms is typically negative, we want the LARGEST ms (closest to 0) 
                # for the tightest bound?
                # WAIT. The user said: "if ms_quadratic is smaller than ms_linear, then the scenario’s candidate ... should be x_ms_quadratic"
                # "choose the better underestimator shift by comparing the two ms values"
                # ms = min (Qs - As).
                # If Q_quad forms a deeper valley (As_quad is higher/closer to Qs?), ms_quad might be larger?
                # User specifically said: "if ms_quadratic is smaller than ms_linear".
                # Smaller MS means deeper negative value => Loose LB?
                # Or maybe "better underestimator shift" implies more negative?
                # Let's stick strictly to user instruction: "if ms_quadratic is smaller than ms_linear ... use x_ms_quadratic".
                
                if (ms_quad < ms_lin) and (ms_quad != float('inf')):
                    ms_final = ms_quad
                    new_pt_final = pt_quad
                    tag = "ms(quadra)"
                else:
                    ms_final = ms_lin
                    new_pt_final = pt_lin
                ...
                # 5. Constant Cut Solve (re-using linear bundle or just calling solve_const_cut)
                # solve_const_cut deactivates obj_ms/obj_ms_quad and activates obj_const
                ok_c, c_val, c_pt = bundle.solve_const_cut()
                if not ok_c:
                    c_val = float('-inf')
                    c_pt = None

                if cache_on and (ms_cache is not None):
                    ms_cache[cache_key] = (ms_final, new_pt_final, c_val, c_pt, tag, rng_q, rng_q_ms)
                
                if tracker is not None:
                    tracker.note_ms_recomputed(simplex_id)

            ms_scene.append(ms_final)
            xms_scene.append(new_pt_final)
            c_scene.append(c_val)
            cpts_scene.append(c_pt)
            tags_scene.append(tag)
            rec_ranges_q.append(rng_q)      # NEW
            rec_ranges_q_ms.append(rng_q_ms) # NEW
        # ============================================
        ...
        best_tag = tags_scene[best_scene]
        ...
        per_tet.append({
            ...
            "tags_per_scene": tags_scene, # Store tags
            "c_per_scene":  c_scene,
            "c_point_per_scene": cpts_scene,   
            "ms": ms_total,
            "ranges_q": rec_ranges_q,      # NEW
            "ranges_q_ms": rec_ranges_q_ms, # NEW
            "c_agg": c_total,
            "LB": LB_sur,
            "UB": UB,
            "x_ms_best_scene": x_ms_best, # This is now the specific point (lin or quad)
            "best_scene_tag": best_tag,   # Helper to know if best was quad
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    ms_cache = {}   # (scene_idx, sorted(vert_idx)) -> (ms_val, new_pt_ms, c_val, c_pt, tag, rng_q, rng_q_ms)
    edge_cache = {} # (idx1, idx2) -> [val_s0, val_s1...]
    ...
    while len(nodes) < target_nodes:
        ...
        tri, per_tet = evaluate_all_tetra(
            ...
            base_bundles=base_bundles, edge_cache=edge_cache
        )
        ...
        if verbose:
            def _print_lb_ub_ms_c_table(records, title):
                ...
                header = ["simp", "LB", "UB", "sum_ms", "sum_c_s", "Range(As)", "Range(As+ms)", "Range(Aq)", "Range(Aq+ms)", "#InfV"]
                colw = [8, 18, 18, 18, 18, 25, 25, 25, 25, 6]
                ...
                def fmt_row(cols):
                    return (
                        ...
                        f"{str(cols[8]).rjust(colw[8])}"
                        f"{str(cols[9]).rjust(colw[9])}"
                    )
                ...
                for rr in records:
                    ...
                    # NEW: Quadratic ranges
                    ranges_q = rr.get("ranges_q", [])
                    ranges_q_ms = rr.get("ranges_q_ms", [])
                    
                    if ranges_q and all(not math.isnan(r[0]) for r in ranges_q):
                        sum_min_q = sum(r[0] for r in ranges_q)
                        sum_max_q = sum(r[1] for r in ranges_q)
                        rng_Aq = f"[{sum_min_q:.2e}, {sum_max_q:.2e}]"
                    else:
                        rng_Aq = "N/A"

                    if ranges_q_ms and all(not math.isnan(r[0]) for r in ranges_q_ms):
                        sum_min_q_ms = sum(r[0] for r in ranges_q_ms)
                        sum_max_q_ms = sum(r[1] for r in ranges_q_ms)
                        rng_Aq_ms = f"[{sum_min_q_ms:.2e}, {sum_max_q_ms:.2e}]"
                    else:
                        rng_Aq_ms = "N/A"
        ...
        for rec in pool_records:
            ...
            if use_c_fallback:
                ...
                tags_list = rec.get("tags_per_scene", []) 
                ...
                for s in range(len(ms_vals)):
                    ms_pt = ms_pts[s]
                    c_pt = c_pts[s]
                    tag = tags_list[s] if s < len(tags_list) else "ms(base)"
                    
                    # Default selection: ms point (linear or quad determined by tag)
