...
def build_pid_model(scenario_name):
    ...
    m.K_p = pyo.Var(domain=pyo.Reals, bounds=[Kp_ref - rp*2**(-k), Kp_ref + rp*2**(-k)])         # controller gain
    m.K_i = pyo.Var(domain=pyo.Reals, bounds=[Ki_ref - ri*2**(-k), Ki_ref + ri*2**(-k)])       # integral gain 
    m.K_d = pyo.Var(domain=pyo.Reals, bounds=[Kd_ref - rd*2**(-k), Kd_ref + rd*2**(-k)])      # dervative gain
