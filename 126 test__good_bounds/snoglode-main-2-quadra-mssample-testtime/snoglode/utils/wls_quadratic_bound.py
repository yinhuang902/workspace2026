...
from snoglode.utils.time_tracker import TimeTracker
...
def compute_wls_quadratic_surrogate_bound(
    ...
    ms_repos: Optional[Dict[Any, MSPointRepo]] = None,
    iteration: int = 0
) -> float:
    """
    ...
    on_F = enabled_methods.get('F', False)  # NEW: Shared random + MS Point Repo method
    ...
    TimeTracker().start("WLSQ_Sampling")
    ...
    TimeTracker().stop("WLSQ_Sampling")
    ...
    if on_E:
        if anchors_arr is not None and len(anchors_arr) > 0:
            TimeTracker().start("WLSQ_E_Sampling")
            ...
            TimeTracker().stop("WLSQ_E_Sampling")
    ...
    # --- Method F: Shared Random Samples + MS Point Repo ---
    samples_F_shared = []
    if on_F:
        # Generate ONE shared random set R of size N_RAND=100
        ...
        for _ in range(num_samples):
            ...
            for i in range(dim):
                ...
                if lb == float('-inf'): lb = -1e4
                if ub == float('inf'): ub = 1e4
                val = rng_shared.uniform(lb, ub)
                v_type = lifted_vars[i]['type']
                if v_type in [SupportedVars.binary, SupportedVars.integers, SupportedVars.nonnegative_integers]:
                    val = round(val)
                    val = max(lb, min(ub, val))
                pt.append(val)
            samples_F_shared.append(np.array(pt))
        if len(samples_F_shared) < num_samples:
             print(f"DEBUG: WLSQ_F shared sampling failed. Got {len(samples_F_shared)}/{num_samples} samples.")
    ...
    # Method F uses per-scenario samples (shared + repo), so we'll init empty and fill later
    if on_F:
        node.lb_problem.plot_data_wlsq['F'] = {
            'interp_points': [], # Will be list of lists of points for ONE scenario (or maybe just the shared ones? Let's store shared for now)
            'interp_truevals': {},
            'ms_point': {},
            'ms_trueval': {}
        }
    ...
    if on_C:
        TimeTracker().start("WLSQ_C_Sampling")
        ...
        TimeTracker().stop("WLSQ_C_Sampling")
    ...
    for subproblem_name in subproblems.names:
        ...
        TimeTracker().start("WLSQ_Shared_TrueValue")
        for i_s, x in enumerate(samples):
            ...
            if success:
                ...
                Y_vals.append(1e6)
        TimeTracker().stop("WLSQ_Shared_TrueValue")
        
        # Evaluate samples_C (Method C)
        Y_vals_C = []
        if on_C:
            TimeTracker().start("WLSQ_C_TrueValue")
            for i_s, x in enumerate(samples_C):
                ...
                if success:
                    ...
                    Y_vals_C.append(1e6)
            TimeTracker().stop("WLSQ_C_TrueValue")
        Y_vals_C = np.array(Y_vals_C)
        
        # Evaluate samples_E (Method E - anchor-mixed)
        Y_vals_E = []
        if on_E and len(samples_E) > 0:
            TimeTracker().start("WLSQ_E_TrueValue")
            for i_s, x in enumerate(samples_E):
                ...
                if success:
                    ...
                    Y_vals_E.append(1e6)
            TimeTracker().stop("WLSQ_E_TrueValue")
        Y_vals_E = np.array(Y_vals_E)
        
        # Evaluate samples_F (Method F - Shared Random + MS Repo)
        Y_vals_F = []
        samples_F_local = []
        ms_points_indices = [] # Track which indices in samples_F_local are from MS repo
        
        if on_F:
            TimeTracker().start("WLSQ_F_TrueValue")
            # 1. Retrieve MS points in bounds
            ms_points_in_bounds = []
            if ms_repos is not None:
                # Try to get repo by name or index
                # We assume ms_repos is keyed by subproblem_name (string) based on solver.py
                if subproblem_name not in ms_repos:
                    ms_repos[subproblem_name] = MSPointRepo(max_points=30, dup_tol=1e-9, bounds_tol=1e-9)
                
                repo = ms_repos[subproblem_name]
                ms_points_in_bounds = repo.get_points_in_bounds(bounds)
            
            # 2. Combine Shared + MS Points
            # Samples_s = R (size 100) + MS_in_bounds_s
            samples_F_local = list(samples_F_shared) # Copy shared
            
            # Add MS points
            start_ms_idx = len(samples_F_local)
            for entry in ms_points_in_bounds:
                samples_F_local.append(entry['x'])
            ...
            for i_s, x in enumerate(samples_F_local):
                ...
                success, obj_val = _solve_true_recourse_primal_gurobi(model)
                if success:
                    Y_vals_F.append(obj_val)
                else:
                    Y_vals_F.append(1e6)
            TimeTracker().stop("WLSQ_F_TrueValue")
        
        Y_vals_F = np.array(Y_vals_F)
        samples_F_local = np.array(samples_F_local) if len(samples_F_local) > 0 else np.empty((0, dim))
        ...
        if 'F' in node.lb_problem.plot_data_wlsq:
            # For F, we have variable number of points per scenario. 
            # We'll store the points used for this scenario if we can, or just the shared ones?
            # The plotter expects 'interp_points' to be consistent or per-scenario?
            # Existing plotter structure assumes 'interp_points' is global for the method.
            # But F has local points. We might need to adjust plotter or just store shared.
            # For now, let's store the LOCAL points for this scenario in a dict keyed by scenario if possible?
            # The current schema is plot_data_wlsq[method]['interp_truevals'][scen] = list.
            # plot_data_wlsq[method]['interp_points'] is a LIST (global).
            # This is a limitation of the current plotter for Method F.
            # We will just store the TRUE VALS for now, and maybe update interp_points to be the shared ones.
            node.lb_problem.plot_data_wlsq['F']['interp_truevals'][subproblem_name] = list(Y_vals_F)
            # We can't easily store per-scenario points in the current global 'interp_points' list.
            # But we can try to store them in a new key if needed.
            # For now, let's just set interp_points to samples_F_shared at the end of the function.
        # --------------------------------------------
        ...
        all_scenario_data.append({
            ...
            'Y_vals_F': Y_vals_F,  # NEW: for Method F
            'samples_F_local': samples_F_local,
            'ms_points_indices': ms_points_indices,
    ...
    local_agg_lb_D2 = 0.0
    local_agg_lb_E = 0.0
    local_agg_lb_F = 0.0 # Fix: Initialize F accumulator  # NEW: for Method E
    ...
    local_beta_agg_F = np.zeros(beta_size)  # NEW: for Method F
    ...
    for data in all_scenario_data:
        ...
        if on_U:
            try:
                TimeTracker().start("WLSQ_Uniform_LeastSquares")
                beta_uni = np.linalg.solve(PhiT_Phi_reg, Phi.T @ Y_vals)
                TimeTracker().stop("WLSQ_Uniform_LeastSquares")
                
                TimeTracker().start("WLSQ_Uniform_Minimization")
                ...
                TimeTracker().stop("WLSQ_Uniform_Minimization")
        ...
        if on_A:
            try:
                TimeTracker().start("WLSQ_A_LeastSquares")
                S = np.sqrt(w_A)
                Phi_w = Phi * S[:, np.newaxis]
                Y_w = Y_vals * S
                PhiT_Phi_w = Phi_w.T @ Phi_w + reg_matrix
                beta_A = np.linalg.solve(PhiT_Phi_w, Phi_w.T @ Y_w)
                TimeTracker().stop("WLSQ_A_LeastSquares")
                
                TimeTracker().start("WLSQ_A_Minimization")
                ...
                TimeTracker().stop("WLSQ_A_Minimization")
        ...
        if on_B:
            try:
                ...
                TimeTracker().stop("WLSQ_B_Minimization")
        ...
        if on_C:
            try:
                TimeTracker().start("WLSQ_C_LeastSquares")
                S_C = np.sqrt(w_C)
                ...
                TimeTracker().stop("WLSQ_C_LeastSquares")
                
                TimeTracker().start("WLSQ_C_Minimization")
                ...
                TimeTracker().stop("WLSQ_C_Minimization")
        ...
        if on_D1:
            try:
                TimeTracker().start("WLSQ_D1_LeastSquares")
                S_D1 = np.sqrt(w_D1)
                ...
                TimeTracker().stop("WLSQ_D1_LeastSquares")
                
                TimeTracker().start("WLSQ_D1_Minimization")
                ...
                TimeTracker().stop("WLSQ_D1_Minimization")
        ...
        if on_D2:
            try:
                TimeTracker().start("WLSQ_D2_LeastSquares")
                S_D2 = np.sqrt(w_D2)
                ...
                TimeTracker().stop("WLSQ_D2_LeastSquares")
                
                TimeTracker().start("WLSQ_D2_Minimization")
                ...
                TimeTracker().stop("WLSQ_D2_Minimization")
        ...
        if on_E and len(Y_vals_E) > 0 and len(samples_E) > 0:
            try:
                ...
                TimeTracker().start("WLSQ_E_LeastSquares")
                PhiT_Phi_E = Phi_E.T @ Phi_E + reg_matrix
                beta_E = np.linalg.solve(PhiT_Phi_E, Phi_E.T @ Y_vals_E)
                TimeTracker().stop("WLSQ_E_LeastSquares")
                
                TimeTracker().start("WLSQ_E_Minimization")
                ...
                TimeTracker().stop("WLSQ_E_Minimization")
        ...
        # --- Fit & Bound: Method F (Shared + MS Repo) ---
        Y_vals_F = data.get('Y_vals_F', np.array([]))
        samples_F_local = data.get('samples_F_local', np.array([]))
        ms_points_indices = data.get('ms_points_indices', [])
        
        if on_F and len(Y_vals_F) > 0 and len(samples_F_local) > 0:
            try:
                # Build Phi_F from samples_F_local
                Phi_F = []
                for x in samples_F_local:
                    row = [1.0]
                    for i in range(dim):
                        row.append(x[i])
                    for i in range(dim):
                ...
                # Uniform weighting
                TimeTracker().start("WLSQ_F_LeastSquares")
                PhiT_Phi_F = Phi_F.T @ Phi_F + reg_matrix
                beta_F = np.linalg.solve(PhiT_Phi_F, Phi_F.T @ Y_vals_F)
                TimeTracker().stop("WLSQ_F_LeastSquares")
                
                # Compute bound AND get MS point details (including residual)
                TimeTracker().start("WLSQ_F_Minimization")
                lb_F, ms_pt, ms_residual, ms_true_val = _compute_bound_from_beta(
                    beta_F, dim, lifted_vars, subproblems, subproblem_name, bounds, model, return_ms_point=True
                )
                TimeTracker().stop("WLSQ_F_Minimization")
                
                local_beta_agg_F += prob * beta_F
                
                # --- REPO UPDATE LOGIC ---
                if ms_repos is not None and subproblem_name in ms_repos:
                    repo = ms_repos[subproblem_name]
                    
                    # 1. Refresh residuals for reused MS points
                    # Q_s(x) = x^T Q x + c^T x + b
                    # We can compute Q_s for all samples using Phi_F @ beta_F
                    Q_vals_F = Phi_F @ beta_F
                    residuals_F = Y_vals_F - Q_vals_F
                    
                    for idx in ms_points_indices:
                        if idx < len(samples_F_local):
                            x_reused = samples_F_local[idx]
                            r_reused = residuals_F[idx]
                            repo.update_residual(x_reused, r_reused, iteration)
                            
                    # 2. Add new MS point
                    # We use the computed ms_residual = F_s(ms_pt) - Q_s(ms_pt)
                    # Option 1: ms_pt is trustworthy (from primal solution)
                    if ms_pt is not None:
                         repo.add_or_update(ms_pt, ms_residual, iteration)

                # Store MS point for method 'F'
                if 'F' in node.lb_problem.plot_data_wlsq:
                    node.lb_problem.plot_data_wlsq['F']['ms_point'][subproblem_name] = ms_pt
                    node.lb_problem.plot_data_wlsq['F']['ms_trueval'][subproblem_name] = ms_true_val
            except Exception as e:
                # print(f"DEBUG: WLSQ_F fitting failed: {e}")
                lb_F = float('-inf')
        else:
            lb_F = float('-inf')
        local_agg_lb_F += prob * lb_F
        
    # Aggregate across ranks
    local_arr = np.array([local_agg_lb_uniform, local_agg_lb_A, local_agg_lb_B, local_agg_lb_C, local_agg_lb_D1, local_agg_lb_D2, local_agg_lb_E, local_agg_lb_F])
    ...
    node.lb_problem.wlsq_F_bound = global_lb_F if on_F else None  # NEW
    
    # --- Compute UB_true for each method ---
    # 1. Aggregate betas
    local_betas = np.concatenate([
        local_beta_agg_uniform, local_beta_agg_A, local_beta_agg_B, 
        local_beta_agg_C, local_beta_agg_D1, local_beta_agg_D2, local_beta_agg_E, local_beta_agg_F
    ])
    ...
    g_beta_F = global_betas[7*bs:8*bs]  # NEW
    
    methods = [
        ('uniform', g_beta_uni), ('A', g_beta_A), ('B', g_beta_B), 
        ('C', g_beta_C), ('D1', g_beta_D1), ('D2', g_beta_D2), ('E', g_beta_E), ('F', g_beta_F)
    ]
    ...
    for m_name, m_beta in methods:
        ...
        # Minimize surrogate
        TimeTracker().start(f"WLSQ_{m_name}_Minimization")
        _, x_star = _minimize_quadratic(m_beta, dim, bounds)
        TimeTracker().stop(f"WLSQ_{m_name}_Minimization")
        ...
        if x_star is not None:
            ...
            if x_tuple in eval_cache:
                ...
                TimeTracker().start(f"WLSQ_{m_name}_TrueValue")
                ...
                TimeTracker().stop(f"WLSQ_{m_name}_TrueValue")
    ...
    return max(global_lb_uniform if on_U else float('-inf'), 
               ...
               global_lb_F if on_F else float('-inf'))
...
def _compute_bound_from_beta(beta, dim, lifted_vars, subproblems, subproblem_name, bounds, model, return_ms_point=False):
    """
    Computes the lower bound from the fitted quadratic surrogate coefficients (beta).
    LB = min_box(Q_s) + min(F_s - Q_s)
    
    If return_ms_point is True, returns (lb, ms_point, ms_residual, ms_true_val).
    Otherwise returns lb.
    """
    # 1. Minimize Q_s over box
    min_Q_val, _ = _minimize_quadratic(beta, dim, bounds)
    if min_Q_val == float('inf'):
        return (float('-inf'), None, None, None) if return_ms_point else float('-inf')
        
    # 2. Minimize (F_s - Q_s) globally
    try:
        b_s = beta[0]
        c_s = beta[1:dim+1]
        quad_coeffs = beta[dim+1:]
        ...
        try:
            ...
            # Option 1: Load solutions if we need the point
            load_sol = return_ms_point
            ms_val = _solve_ms_dual_bound_gurobi(model, 'temp_ms_obj', load_solutions=load_sol)
            
            if return_ms_point and ms_val != float('-inf'):
                # Extract MS point from loaded solution
        ...
        if return_ms_point:
            # We already extracted ms_point above if success
            if ms_point is None:
                 return float('-inf'), None, None, None
            
            # Compute true recourse at MS point for reporting
            
            # 1. Save state of ALL lifted vars locally
            saved_state_local = []
            for var in subproblems.subproblem_lifted_vars[subproblem_name]:
                saved_state_local.append((var, var.lb, var.ub, var.value, var.is_fixed()))

            # 2. Fix vars to MS point
            ...
            # 3. Compute true value
            success, ms_true_val = _solve_true_recourse_primal_gurobi(model)
            if not success:
                ms_true_val = float('inf')
                
            # 4. Restore vars to EXACT original state
            for (var, lb_v, ub_v, val_v, fixed_v) in saved_state_local:
                var.lb = lb_v
                var.ub = ub_v
                if fixed_v:
                    var.fix(val_v)
