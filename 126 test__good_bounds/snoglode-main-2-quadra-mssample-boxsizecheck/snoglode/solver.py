...
# --- Volume Threshold Switch for LB Method Selection ---
# If box volume <= this, use WLSQ LB; otherwise use original SNoGloDe LB.
# Set to +inf to always use WLSQ (if enabled methods exist).
# Set to 0 to always use original (never trigger WLSQ).
VOLUME_SWITCH_THRESHOLD = float(500)  # default: always WLSQ when enabled
...
class Solver():
    ...
    def dispatch_lb_solve(self,
        ...
        # Compute current box volume for volume-threshold switch
        box_vol = _compute_box_volume(current_node.state)
        use_wlsq = (box_vol <= VOLUME_SWITCH_THRESHOLD)
        
        # Debug print: which LB method is selected based on volume
        if rank == 0:
            method_str = "WLSQ" if use_wlsq else "Original"
            print(f"[LB_SWITCH] iter={self.iteration}, box_vol={box_vol:.6e}, threshold={VOLUME_SWITCH_THRESHOLD:.6e}, using={method_str}")
        
        # Initialize all optional bounds to None
        ...
        if use_wlsq:
            # --- WLSQ path: run WLSQ lower bound(s) ---
            # Still need to run the original LB solve to get the basic objective/feasibility
            self.lower_bounder.solve(subproblems = self.subproblems,
                                     node = current_node)
            
            # Construct config dicts for WLSQ methods
            wlsq_methods = {
                'uniform': ENABLE_WLSQ_UNIFORM,
                'A': ENABLE_WLSQ_A,
                'B': ENABLE_WLSQ_B,
            ...
            # --- Original path: run original SNoGloDe lower bound ---
            self.lower_bounder.solve(subproblems = self.subproblems,
                                     node = current_node)
            
            # Optionally compute other experimental bounds (Quadratic, Random PID)
            if ENABLE_QUAD_LB:
                try:
