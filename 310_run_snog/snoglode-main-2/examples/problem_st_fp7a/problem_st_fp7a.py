"""
problem_st_fp7a.py -- Snoglode solver for SNGO-master/Global/st_fp7a

Julia: NS=100, nfirst=5, nparam=5, seed=1234
Model: 20 vars (xi>=0), 10 <= constraints, Obj: Min sum(2xi - 0.5xi^2)
...
NUM_SCENARIOS = 5; NFIRST = 5; NPARAM = 5; SEED = 1234
...
def build_scenario_model(scenario_name):
    ...
    m.obj=pyo.Objective(expr=sum(2*x[i]-0.5*x[i]**2 for i in range(20)),sense=pyo.minimize)
...
if __name__=='__main__':
    ...
    if size==1:params.set_logging(fname=os.path.join(_log_dir,"problem_st_fp7a_log"))
    ...
    # ---------------------------------------------------------
    csv_filename=os.path.join(_log_dir,"problem_st_fp7a_result.csv")
