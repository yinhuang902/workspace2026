...
function ParPipsNlp_solve(master::JuMP.Model)
	 ...
	 removed = removeConnection(master, connectid)
	...
	function str_eval_g(nodeid,x0,x1,new_eq_g, new_inq_g)
	 ...
	 master.linconstr = [master.linconstr; removed]
	 root = 0
    	 r = MPI.Comm_rank(comm)
...
function removeConnection(master::JuMP.Model, deleterow)
    ...
    #deleted = copy(master.linconstr[deleterow]) #map(c->copy(c, master), master.linconstr[deleterow])    
