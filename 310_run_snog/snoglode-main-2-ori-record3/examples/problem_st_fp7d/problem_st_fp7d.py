"""
problem_st_fp7d.py -- Snoglode solver for SNGO-master/Global/st_fp7d

Julia: NS=100, nfirst=5, nparam=5, seed=1234
Model: 20 vars (xi>=0), 10 <= constraints (same as st_fp7a/2_1_7)
Objective: Min sum(8*xi - 0.5*xi^2, i=1..20)
...
def build_scenario_model(scenario_name):
    ...
    # Objective: Min sum(8*xi - 0.5*xi^2, i=1..20)
    m.obj=pyo.Objective(expr=sum(8*x[i] - 0.5*x[i]**2 for i in range(20)),sense=pyo.minimize)
...
if __name__=='__main__':
    ...
    if size==1:params.set_logging(fname=os.path.join(_log_dir,"problem_st_fp7d_log"))
    else:params.set_logging(fname=os.path.join(_log_dir,"problem_st_fp7d_log_parallel"))
    if rank==0:params.display()
    solver=sno.Solver(params)
    # ---------------------------------------------------------
    # CSV Logging Implementation (Monkey Patch)
    # ---------------------------------------------------------
    csv_filename=os.path.join(_log_dir,"problem_st_fp7d_result.csv")
