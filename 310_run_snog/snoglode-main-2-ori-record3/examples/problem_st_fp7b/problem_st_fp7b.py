"""
problem_st_fp7b.py -- Snoglode solver for SNGO-master/Global/st_fp7b

Julia: NS=100, nfirst=5, nparam=5, seed=1234
Model: 20 vars (xi>=0), 10 <= constraints (same as st_fp7a/2_1_7)
Objective: Min sum(-0.5*xi^2 - 5*xi, i=1..20)
...
NUM_SCENARIOS = 10; NFIRST = 5; NPARAM = 5; SEED = 1234
...
def build_scenario_model(scenario_name):
    ...
    for i in range(1,21):setattr(m,f'x{i}',pyo.Var(bounds=(0,None),initialize=0))
    x=[getattr(m,f'x{i}') for i in range(1,21)]
    _add_constraints(m,x,rhs)
    # Objective: Min sum(-0.5*xi^2 - 5*xi, i=1..20)
    m.obj=pyo.Objective(expr=sum(-0.5*x[i]**2 - 5*x[i] for i in range(20)),sense=pyo.minimize)
...
if __name__=='__main__':
    ...
    if size==1:params.set_logging(fname=os.path.join(_log_dir,"problem_st_fp7b_log"))
    else:params.set_logging(fname=os.path.join(_log_dir,"problem_st_fp7b_log_parallel"))
    if rank==0:params.display()
    solver=sno.Solver(params)
    # ---------------------------------------------------------
    # CSV Logging Implementation (Monkey Patch)
    # ---------------------------------------------------------
    csv_filename=os.path.join(_log_dir,"problem_st_fp7b_result.csv")
