"""
==============================================================
Two-Scenario Cubic Optimization — Quadratic Surrogate
==============================================================
...
Algorithm: Iterative piecewise quadratic underestimator splitting.
"""
...
def compute_ms_shift(v_func, a: float, b: float, c: float, Y: Tuple[float, float]) -> Tuple[float, float]:
    """Computes ms = min(v_s - A_s) using scipy minimize_scalar."""
    lo, hi = Y
...
def run_iterative_splitting(Y0: Tuple[float, float], num_iterations: int = 20):
    output_log_file = os.path.join(SCRIPT_DIR, "cubic_quad_iteration_summary.txt")
    with open(output_log_file, "w") as f:
        f.write("=== Cubic Problem — Quadratic Surrogate Iterative Splitting Log ===\n")
    ...
    for k in range(num_iterations):
        ...
        # UB: min of expected_v at all segment endpoints
        all_endpoints = set()
        ...
        # LB: min of all segment local LBs
        ...
        axes[2].legend()
        plt.tight_layout()
        plt.savefig(os.path.join(SCRIPT_DIR, f'cubic_quad_iteration_{k}.png'), dpi=150)
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    print("Results saved as cubic_quad_iteration_summary.txt and PNG files generated.")
