...
class Segment:
    Y: Tuple[float, float]
    # Local LINEAR models for scene 1 and 2: L_s(y) = b*y + c  (a=0 always)
    a1: float  # always 0 for linear
    b1: float  # slope
    c1: float  # intercept
    ms1: float
    y_ms1: float
    
    a2: float  # always 0 for linear
    b2: float  # slope
    c2: float  # intercept
    ms2: float
    y_ms2: float
...
def fit_linear_interpolant(v_func, Y: Tuple[float, float]) -> Tuple[float, float, float]:
    """
    Fits L_s(y) = b*y + c using the two endpoints of interval Y.
    Returns (a, b, c) where a=0 for compatibility with the quadratic interface.
    """
    lo, hi = Y
    f_lo = v_func(lo)
    f_hi = v_func(hi)
    ...
    b = (f_hi - f_lo) / (hi - lo)  # slope
    c = f_lo - b * lo               # intercept
    a = 0.0                          # no quadratic term
    return a, b, c

def compute_ms_shift(v_func, a: float, b: float, c: float, Y: Tuple[float, float], n_grid: int = 5000) -> Tuple[float, float]:
    """
    Computes ms = min(v_s - L_s) using Gurobi.
    v_s is either -sqrt(|y|) or sqrt(|y|).
    L_s(y) = b*y + c  (a=0 for linear).
    We minimize v_s(y) - L_s(y) over y in Y.
    ...
    for (sub_lo, sub_hi, sign) in sub_intervals:
        try:
            ...
            # v_s(y) = ±s, L_s(y) = a*y^2 + b*y + c (a=0 for linear)
            # objective: minimize v_s(y) - L_s(y)
            if is_neg_sqrt:
...
def min_of_linear_on_interval(a: float, b: float, c: float, Y: Tuple[float, float]) -> float:
    """Finds the minimum of a*y^2 + b*y + c over interval Y. For linear (a=0), it's min at endpoints."""
    lo, hi = Y
...
def run_iterative_splitting(Y0: Tuple[float, float], num_iterations: int = 20):
    output_log_file = "p4_linear_iteration_summary.txt"
    with open(output_log_file, "w") as f:
        f.write("=== P4 Linear Iterative Splitting Log ===\n")
    ...
    for k in range(num_iterations):
        ...
        for i, seg in enumerate(active_segments):
            ...
            # Linear: a=0, so a*y^2 + b*y + c = b*y + c
        ...
        plt.tight_layout()
        plt.savefig(f'p4_linear_iteration_{k}.png', dpi=150)
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    print(f"Results saved as p4_linear_iteration_summary.txt and PNG files generated.")
