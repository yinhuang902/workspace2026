...
from wls_fit import compute_wls_fverts_for_scene, fit_wls_affine, eval_affine
...
def evaluate_all_tetra(nodes, scen_values, ms_bundles, first_vars_list,
                       ...
                       base_bundles=None,  # scenario bundles for WLS Q evaluation
                       wls_fverts_cache=None):  # simplex_id → fverts_per_scene (persistent across iters)
    """
    ...
    for k, simp in enumerate(simplices):
        ...
        fverts_per_scene_raw = [[scen_values[s][i] for i in idxs] for s in range(S)]

        # ==========  CANONICAL WLS As CONSTRUCTION  ==========
        # As is ALWAYS WLS-based when base_bundles is provided.
        # wls_fverts_cache persists across iterations:
        #   - Iter 0: all simplices are new → compute WLS, store in cache
        #   - Iter k>=1: only new children need WLS; old simplices reuse cache
        # ms_cache is always used (fverts are stable per simplex once cached).
        # ==========================================================
        _wls_active = (base_bundles is not None)
        key_base = tuple(sorted(idxs))

        # ---- Determine fverts_per_scene: cache hit or WLS compute ----
        _wls_cache_hit = (
            _wls_active
            and (wls_fverts_cache is not None)
            and (key_base in wls_fverts_cache)
        )

        wls_n_qualifying = None  # per-scenario list of qualifying point counts

        if _wls_cache_hit:
            # Reuse previously computed WLS fverts — no evaluate_Q_at calls
            fverts_per_scene, wls_n_qualifying = wls_fverts_cache[key_base]
        elif _wls_active:
            # New simplex: compute WLS fverts from scratch
            # First solve ms/cs with raw fverts to get c_pts for avg-cs
            _pre_cpts = []
            for _ws in range(S):
                _, _, _, _c_pt = ms_on_tetra_for_scene(
                    ms_bundles[_ws], verts, fverts_per_scene_raw[_ws]
                )
                _pre_cpts.append(_c_pt)

            fverts_per_scene = list(fverts_per_scene_raw)  # default fallback
            wls_n_qualifying = [0] * S
            try:
                _wls_centroid = tuple(np.mean(verts, axis=0))
                _wls_valid_cpts = [
                    pt for pt in _pre_cpts
                    if pt is not None and all(math.isfinite(v) for v in pt)
                ]
                _wls_avg_cs = (
                    tuple(np.mean(_wls_valid_cpts, axis=0))
                    if _wls_valid_cpts else None
                )

                _wls_q_centroid = []
                _wls_q_avg_cs   = []
                for _ws in range(S):
                    _wls_q_centroid.append(
                        evaluate_Q_at(base_bundles[_ws], first_vars_list[_ws],
                                      _wls_centroid)
                    )
                    if _wls_avg_cs is not None:
                        _wls_q_avg_cs.append(
                            evaluate_Q_at(base_bundles[_ws], first_vars_list[_ws],
                                          _wls_avg_cs)
                        )

                _wls_extra_pts = [_wls_centroid]
                _wls_extra_q_tmpl = [_wls_q_centroid]
                if _wls_avg_cs is not None:
                    _wls_extra_pts.append(_wls_avg_cs)
                    _wls_extra_q_tmpl.append(_wls_q_avg_cs)

                _wls_fverts = []
                _wls_changed = False
                for _ws in range(S):
                    _extra_q = [eq[_ws] for eq in _wls_extra_q_tmpl]
                    # Count qualifying points (Q < 2) for this scenario
                    _all_q = list(fverts_per_scene_raw[_ws]) + _extra_q
                    wls_n_qualifying[_ws] = sum(1 for q in _all_q if q < 2.0)
                    _pred = compute_wls_fverts_for_scene(
                        verts, fverts_per_scene_raw[_ws],
                        _wls_extra_pts, _extra_q,
                        q_threshold=2.0,
                    )
                    if _pred is not None:
                        _wls_fverts.append(_pred)
                        _wls_changed = True
                    else:
                        _wls_fverts.append(list(fverts_per_scene_raw[_ws]))

                if _wls_changed:
                    fverts_per_scene = _wls_fverts
            except Exception:
                pass  # keep raw fallback

            # Store in cache (even if fallback — avoids re-computing next iter)
            if wls_fverts_cache is not None:
                wls_fverts_cache[key_base] = (fverts_per_scene, wls_n_qualifying)
        else:
            # WLS not active — use raw vertex values
            fverts_per_scene = fverts_per_scene_raw

        # ---- ms/cs solve with final (cached or freshly computed) fverts ----
        ms_scene = []
        xms_scene = []
        c_scene = []
        cpts_scene = []
        ...
        # ==========  end single-path As construction  ==========
        ...
        per_tet.append({
            ...
            "wls_n_qualifying": wls_n_qualifying,  # per-scenario qualifying point counts (Q<2)
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    wls_fverts_cache = {}  # simplex_id -> fverts_per_scene (persistent across iterations)
    ...
    while len(nodes) < target_nodes:
        ...
        tri, per_tet = evaluate_all_tetra(
            ...
            iter_num=it,
            base_bundles=base_bundles,   # WLS Q evaluation
            wls_fverts_cache=wls_fverts_cache,
        )
                ...
                if _ipopt_r and _ipopt_r.get("ok"):
                    ...
                    if _ipopt_r.get("K") is not None:
                        ...
                        try:
                            ...
                            _bary_rest = np.linalg.solve(_T, _K_pt - _sv[0])  # (3,)
        ...
        location_str = "N/A"  # default; set below if applicable
        if selected_rec_before_split is not None:
            ...
            for new_idx in new_simplex_indices:
                if new_idx in per_tet_dict:
                    ...
                    _child_c_finite = [x for x in _child_c if math.isfinite(x)]
                    _child_avg_cs = np.mean(_child_c_finite) if _child_c_finite else float('nan')
                    _child_wls_nq = child_rec.get("wls_n_qualifying")
                    _nq_str = str(_child_wls_nq) if _child_wls_nq is not None else "N/A"
                    _diag_lines.append(f"    T{new_idx} {child_id}: LB = {child_lb/S:.9f}, avg_cs = {_child_avg_cs:.9f}, wls_nq = {_nq_str}")
