    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    while len(nodes) < target_nodes:
        ...
        if verbose:
            ...
            if _geom_containing:
                ...
                for _si, _lam in _geom_containing:
                    ...
                    print(f"  sum(c_s_dual)={sum(_m.get('dual_bound',0) or 0 for _m in _cs_metas):.9f}, "
                    ...
                    for _s_idx in range(S):
                        ...
                        _cs_dual_s = _cs_metas[_s_idx].get("dual_bound", None) if _s_idx < len(_cs_metas) else None
                        _cs_prim_s = _cs_metas[_s_idx].get("primal_obj", None) if _s_idx < len(_cs_metas) else None
                    ...
                    print(f"    SUM Q_s(K_EF)={_sum_q_ef:.9f}, sum(c_s)={sum(_m.get('dual_bound',0) or 0 for _m in _cs_metas):.9f}, "
                          f"diff={_sum_q_ef - sum(_m.get('dual_bound',0) or 0 for _m in _cs_metas):.6e}")
                print()
            else:
        ...
        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([f"{iter_time:.3f}", n_nodes, f"{lb_val:.9f}", f"{ub_val:.9f}", f"{rel_gap*100:.7f}%", f"{abs_gap:.5f}", f"{lb_ever:.9f}", f"{ub_ever:.9f}"])
