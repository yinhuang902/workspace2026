...
class UpperBounder(BoundingProblemBase):
    ...
    def solve_subproblem(self, 
        ...
        # Robust handling: try to recover an incumbent, else return inf (valid but weak UB)
        else:
            try:
                subproblem_model.solutions.load_from(results)
                obj = pyo.value(get_active_objective(subproblem_model))
                if obj is not None:
                    return True, obj
            except:
                pass
            
            # Warn and return infinity (do not mark as infeasible unless solver said Infeasible)
            logging.warning(f"UB subproblem terminated with {results.solver.termination_condition}. No incumbent. Returning inf.")
