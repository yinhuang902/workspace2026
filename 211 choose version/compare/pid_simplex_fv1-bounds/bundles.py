...
class MSBundle:
    ...
    def update_tetra(self, tet_vertices, fverts_scene):
        ...
        # (critical for McCormick envelope quality on bilinear terms)
        self.Kp.setlb(min(vx));  self.Kp.setub(max(vx));  self.gp.update_var(self.Kp)
        self.Ki.setlb(min(vy));  self.Ki.setub(max(vy));  self.gp.update_var(self.Ki)
        self.Kd.setlb(min(vz));  self.Kd.setub(max(vz));  self.gp.update_var(self.Kd)
