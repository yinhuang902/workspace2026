...
def cs_only_on_tetra_for_scene(ms_bundle, tet_vertices, fverts_scene):
    ...
    Returns the same 4-tuple as ms_on_tetra_for_scene:
        (ms_val=+inf, new_pt_ms=None, c_val, c_pt)
    """
    ms_bundle.update_tetra(tet_vertices, fverts_scene)
...
# status in {"full", "cs_only", "ms_only", "ms_failed", "cs_failed", "both_failed"}
...
def ensure_ms_for_simplex(rec, ms_bundles, nodes, scen_values, ms_cache,
    ...
    for w in range(S):
        ...
        # Check if cache already has usable MS
        ...
        if ms_cache is not None and cache_key in ms_cache:
            ...
            if _cache_status_has_ms(entry[0]):
                # Reuse MS from cache
...
def ensure_cs_for_simplex(rec, ms_bundles, nodes, scen_values, ms_cache,
    ...
    for w in range(S):
        ...
        if not already_has_cs:
            _, _, c_val, c_pt = cs_only_on_tetra_for_scene(
                ms_bundles[w], verts, fverts_per_scene[w]
            )
            c_scene[w] = c_val
            cpts_scene[w] = c_pt
    ...
    LB_sur = solve_surrogate_lb_for_tet(fverts_per_scene2, ms_scene, c_scene)
    rec["LB"] = LB_sur
    if lb_sur_cache is not None:
        lb_sur_cache[key_base] = LB_sur
...
def evaluate_all_tetra(nodes, scen_values, ms_bundles, first_vars_list,
    ...
    for k, simp in enumerate(simplices):
        ...
        # Compute readiness flags for this record
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    while len(nodes) < target_nodes:
        ...
        _cs_only_iter = (it == 0 and initial_nodes is None)  # box start: CS-only first round
        try:
            if _cs_only_iter:
                # CS-only evaluation for first iteration (box start)
                if verbose:
                    print(f"[Iter {it}] CS-only evaluation (box start): skipping MS solves")
                tri, per_tet = evaluate_all_tetra_cs_only(
        ...
        # Lazy MS: if selected simplex has no MS, compute it now
        ...
        if it == 0:
            try:
                _i0_lines = []
                _i0_lines.append("=" * 70)
                _i0_eval_mode = "CS-only (box start)" if _cs_only_iter else "full MS+CS"
                _i0_lines.append(f"[Iter 0] SIMPLEX SUMMARY  (eval_mode={_i0_eval_mode})")
