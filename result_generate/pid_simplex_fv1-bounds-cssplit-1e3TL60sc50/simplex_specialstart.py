...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    while len(nodes) < target_nodes:
        ...
        ub_start = UB_incumbent  # snapshot before any updates this iteration
        ...
        # 9) Split point: avgCS from the selected (min-LB) simplex
        # ------------------------------------------------
        _t_phase = perf_counter()
        new_node = None
        chosen_ms = None
        ...
        cs_pts = lb_simp_rec.get("c_point_per_scene", [])
        valid_cs = [pt for pt in cs_pts
                    if pt is not None and all(math.isfinite(v) for v in pt)]

        if len(valid_cs) >= 2:
            avg_cs_point = tuple(np.mean(valid_cs, axis=0))
            pt_source = "avgCS"
        else:
            # Fallback: centroid of simplex vertices
            avg_cs_point = tuple(np.mean(np.array(lb_simp_rec["verts"]), axis=0))
            pt_source = "centroid"

        if verbose:
            print(f"[Iter {it}] Split point ({pt_source}): "
                  f"{tuple(round(float(x), 6) for x in avg_cs_point)}")

        # Collision check + snap
        if min_dist_to_nodes(avg_cs_point, nodes) >= min_dist:
            cand_pt_pert, loc_type, loc_info = _snap_feature(avg_cs_point, lb_simp_rec)
            ...
            if verbose:
                print(f"  -> Accepted. loc_type={loc_type}")
        else:
            stop_due_to_collision = True
            if verbose:
                d = min_dist_to_nodes(avg_cs_point, nodes)
                print(f"  -> Rejected: too close to existing node "
                      f"(dist={d:.3e} < {min_dist:g})")
        ...
        if enable_3d_plot and (plot_every is not None) and (it % plot_every == 0):
            ...
            # Use the actual chosen point (new_node) if available, otherwise fallback to avg_cs_point
            pt_to_plot = new_node if new_node is not None else avg_cs_point
        ...
        if verbose and chosen_cand is not None and chosen_cand.get("scene", -1) >= 0:
            ...
            pt_type = chosen_cand.get("pt_source", "avgCS")
        ...
        # Overall UB refresh flag: did UB_incumbent strictly decrease this iteration?
        overall_ub_updated = (UB_incumbent < ub_start - 1e-12)
        ...
        _final_ub_src = ub_source_end if overall_ub_updated else ub_source_current
        _final_ub_simplex = ub_simplex_id_this_iter if overall_ub_updated else ub_simplex_id_current
        try:
        ...
        if _ef_log_row is not None:
            _ef_log_row["ub_updated"] = overall_ub_updated
        ...
        if _i0_deferred is not None:
            try:
                ...
                _i0_deferred.append(f"    UB updated this iter: {overall_ub_updated}")
        ...
        if _diag_deferred is not None:
            try:
                ...
                _diag_deferred.append(f"  UB updated this iter: {overall_ub_updated}")
        ...
        try:
            with open(dbg_ef_solver_path, "a", encoding="utf-8") as _fef:
                ...
                _overall_ub_updated = overall_ub_updated
                _fef.write(
        ...
        try:
            ...
            ef_info = {
                ...
                "used_for_UB": overall_ub_updated,
            }

            # UB info
            ub_info = {"updated_this_iter": overall_ub_updated}
