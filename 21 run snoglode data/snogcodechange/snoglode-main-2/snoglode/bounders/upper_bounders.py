...
class UpperBounder(BoundingProblemBase):
    ...
    def solve(self, 
        ...
        The "upper bounding" problem
         ...
         (2) adjust complicating variable bounds to reflect current node state
        ...
        node : Node
            ...
            and fixed states of all complicating variables.
        subproblems : Subproblems
            initialized subproblems.
        candidate_solution : dict            
            for each of the complicating variable names, an associated value to fix solution to.
        ...
        for subproblem_name in subproblems.names:
            ...
            self.fix_to_candidate_solution(subproblem_complicating_variables = subproblems.subproblem_complicating_vars[subproblem_name], 
        ...
        node.ub_problem.is_feasible(pyo.value(statistics.aggregated_objective), candidate_solution)

        return True

    
    def fix_to_candidate_solution(self, 
                                  subproblem_complicating_variables: list, 
        ...
        for var in subproblem_complicating_variables:
...
class AbstractCandidateGenerator(BoundingProblemBase):
    ...
    def __init__(self, 
        ...
        # grab all complicating var IDs - need to return a dict with these values
        # from the generate() function
        self.complicating_var_ids = subproblems.complicating_var_ids
    ...
    def generate(self, 
        ...
        candidate_solution : dict
            For each of the complicating variable ID's, therre should be a 
...
class AverageLowerBoundSolution(AbstractCandidateGenerator):
    ...
    def generate_candidate(self, 
        ...
        """
        avg_solution = compute.average_lb_solution(node, subproblems)

        # check that we had a solution to compute all of this info
        has_none_value = any(value is None for value in avg_solution.values())
        return [not has_none_value, avg_solution, math.nan]
...
class SolveExtensiveForm(AbstractCandidateGenerator):
    ...
    def generate_ef_candidate(self,
                              subproblems: Subproblems) -> Tuple[bool, dict, float]:
        """
        ...
        if (size > 1):
            ...
            sol = {}
            var_val = None
            for complicating_var_id in subproblems.ef.complicating_var_ids:
                if (rank==best_rank):
                    var_val = pyo.value(subproblems.ef.model.complicating_vars[complicating_var_id])
                
                var_val = MPI.COMM_WORLD.bcast(obj = var_val,
                                               root = best_rank)
                sol[complicating_var_id] = var_val
    ...
    def generate_candidate(self, 
                           ...
                           subproblems: Subproblems) -> Tuple[bool, dict, float]:
        """
        Use local solution of the EF as candidate.
        ...
        candidate_solution_obj : float
            ...
            to be produced.
        """
        return self.generate_ef_candidate(subproblems)