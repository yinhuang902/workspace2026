...
class MockCandidateGenerator(AbstractCandidateGenerator):
    def __init__(self, 
                 solver, 
                 subproblems: sno.Subproblems) -> None:
...
def bilinear_subproblem_creator(subproblem_name):
    ...
    complicating_variable_ids = {"x": model.x}
...
class IntegerProgram():
    ...
    def ip_subproblem_creator(self, subproblem_name):
        ...
        # grab the list of first stage variables
        complicating_variable_ids = {("y", i): model.y[i] \
                                    for i in [1,2,3]}
        self.complicating_variable_ids = complicating_variable_ids
...
def pmedian_subproblem_creator(subproblem_name):
    ...
    # grab the list of first stage variables
    complicating_variable_ids = {("facility", i): model.x[i] \
...
def integer_knapsack_subproblem_creator(_):
    ...
    # grab the list of first stage variables
    complicating_variable_ids = {("item", i): m.x[i] \
...
def quadradic_subproblem_creator(scenario_name):
    name_value_map = {
    'scenario1': 0,
    'scenario2': 1,
    'scenario3': 2
