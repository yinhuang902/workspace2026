...
class LGLowerBounder(AbstractLowerBounder):
    ...
    def solve(self, 
        ...
        # Storage for Cuts on Rank 0
        # List of (scenario_name, mu_vector_for_scenario, subproblem_obj_val)
        cuts = []  
        ...
        for k in range(self.K):
            ...
            if rank == 0:
                ...
                for name in global_results:
                    ...
                    cuts.append( (name, cp.deepcopy(current_mu[name]), v_val) )
        ...
        if rank == 0:
            ...
            if len(scenarios_no_cuts) > 0:
                ...
                rmp_feasible, rmp_obj = self._solve_rmp(node, subproblems, cuts, all_var_ids, all_scenario_names)
        ...
        if rmp_feasible:
            # PATCH 1.4: (C) Simplified stats - only set aggregated_objective
            # Do NOT populate per-scenario stats from local_results as it may be
            # partial/empty if we broke early from inner loop
            # Use RMP objective as the node lower bound
    ...
    def _solve_rmp(self, node, subproblems, cuts, all_var_ids, all_scenario_names):
        """
        ...
        # Objective: sum(p_s * eta_s)
        # Probabilities needed. `subproblems.probability` only has local?
        # `subproblems` should have global probability info?
        # `subproblems` init iterates all names. `probability` dict key is local `names`.
        # Wait, `__init__` initializes `self.probability` for `self.names`.
        # Does Rank 0 know ALL probabilities?
        # `subproblems.probability` on Rank 0 only has Rank 0's scenarios.
        # We need to gather probabilities or assume uniform 1/N.
        # Requirement: "uniform p_omega if not provided".
        # Safe assumption: use 1/N.
        p_val = 1.0 / len(all_scenario_names)
        m.obj = pyo.Objective(expr=sum(p_val * m.eta[s] for s in all_scenario_names), sense=pyo.minimize)
        ...
        # Solve
        # Use simple solver
        opt = pyo.SolverFactory('gurobi') # Or whatever is available, usually gurobi
