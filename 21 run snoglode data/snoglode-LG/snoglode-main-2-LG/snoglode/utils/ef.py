...
class ExtensiveForm():
    ...
    def __init__(self, 
        ...
        NOTE (2): complicating vars are found via self.model.complicating_vars (which is 
        an indexed pyo.Var, where each index corresponds to a particular complicating_var_id)
        These complicating vars have domains of pyo.Any; this may be changed in the future.
        ...
        """ Build the EF complicating variables """

        # get all of the variable IDs
        self.complicating_var_ids = []
        for var_type in SupportedVars:
            complicating_var_ids = list(subproblems.root_node_state[var_type].keys())
            if (complicating_var_ids): self.complicating_var_ids += complicating_var_ids

        # create the first stage variables, indexed by the var_ids
        complicating_vars = pyo.Var(self.complicating_var_ids,
                              name = "complicating_vars",
                              domain = pyo.Reals)
        self.model.add_component("complicating_vars", complicating_vars)
        ...
        for subproblem_name in subproblems.all_names:
            ...
            if (subproblem_name not in subproblems.names):
                ...
                # add the ref to pyo.Var scenario-specific complicating var to dict
                for subproblem_complicating_var in self.other_subproblems.subproblem_complicating_vars[subproblem_name]:

                    # extract var ID
                    _, complicating_var_id, _ = self.other_subproblems.var_to_data[subproblem_complicating_var]

                    # grab the EF specific first stage variables
                    ef_complicating_var = self.model.complicating_vars[complicating_var_id]

                    # add nonant for scen / first stage var to list
                    self.model.nonants[subproblem_name, complicating_var_id] = \
                        (ef_complicating_var == subproblem_complicating_var)
            ...
            if (subproblem_name in subproblems.names):
                ...
                # add the ref to pyo.Var scenario-specific complicating var to dict
                for subproblem_complicating_var in subproblems.subproblem_complicating_vars[subproblem_name]:

                    # extract var ID
                    _, complicating_var_id, _ = subproblems.var_to_data[subproblem_complicating_var]
    ...
    def save_solution(self) -> dict:
        ...
        candidate_solution_state : dict
            dict of keys which correspond to the complicating variable ID
            and the element is the value of that complicating variable ID.
        """
        candidate_solution_state = {}
    
        # for each of the first stage variables, save value
        for complicating_var_id in self.complicating_var_ids:

            # grab the first stage var ref
            ef_complicating_var = self.model.complicating_vars[complicating_var_id]
            
            # grab value, save under var_id
            candidate_solution_state[complicating_var_id] = pyo.value(ef_complicating_var)
