...
class MostInfeasibleBinary(SelectionStrategy):
    ...
    def select_variable(self, 
        ...
        # if we have run out of binaries, move on to branching the rest
            ...
            if len(vars) == 0:
                raise ValueError("No candidate variables available for branching")

            index = MPI.COMM_WORLD.bcast(np.random.randint(0, len(vars)), root=0)
            var_name = vars[index]

            # Determine var type using offset logic:
            #   reals occupy indices      [0, len(reals))
            #   integers occupy indices   [len(reals), len(reals)+len(integers))
            #   nonneg_ints occupy rest   [len(reals)+len(integers), len(vars))
            if index < len(reals):
                var_type = SupportedVars.reals
            elif index < len(reals) + len(integers):
                var_type = SupportedVars.integers
            else:
                var_type = SupportedVars.nonnegative_integers
...
class Pseudocost(SelectionStrategy):
    ...
    def select_variable(self, 
        ...
        # Note: root node doesn't have 'dir' attribute, use getattr with default
        if getattr(node, 'dir', NodeDirection.root) != NodeDirection.root and node.lb_problem.feasible:
...
if __name__ == "__main__":
    """
    Sanity check: test the branching variable offset logic used in
    MostInfeasibleBinary (and similar code in other strategies).
    Samples many times and verifies returned var_name always belongs
    to the union of the three lists and never raises IndexError.
    """
    import random as _random
    _random.seed(12345)

    def _simulate_branching_selection(reals, integers, nonnegative_integers, n_samples=10000):
        """
        Simulates the fixed branching variable selection logic.
        Returns True if all samples are valid, raises assertion otherwise.
        """
        all_vars = reals + integers + nonnegative_integers
        total = len(all_vars)

        if total == 0:
            print("Sanity check: empty list case - correctly would raise ValueError. PASS")
            return True

        for _ in range(n_samples):
            # Simulate uniform random index selection
            idx = _random.randint(0, total - 1)
            ...
            if idx < len(reals):
                expected_list = reals
                var_type = "reals"
            elif idx < len(reals) + len(integers):
                expected_list = integers
                var_type = "integers"
            else:
                expected_list = nonnegative_integers
                var_type = "nonnegative_integers"

            # Verify var_name belongs to the expected list
            assert var_name in expected_list, \
                f"var_name={var_name} not in {var_type}: {expected_list}"
            # Verify var_name is in the union
            assert var_name in all_vars, \
                f"var_name={var_name} not in union of all variable lists"
    ...
    print("Running branching variable selection sanity checks...")
    for i, (reals, integers, nonneg) in enumerate(test_cases):
        try:
            _simulate_branching_selection(reals, integers, nonneg)
            print(f"  Test case {i+1}: reals={len(reals)}, ints={len(integers)}, nonneg={len(nonneg)} - PASS")
        except (IndexError, AssertionError) as e:
            print(f"  Test case {i+1}: FAIL - {e}")
            raise
