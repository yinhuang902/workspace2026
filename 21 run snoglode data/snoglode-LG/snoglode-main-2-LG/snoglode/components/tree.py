...
from snoglode.utils.cut_pool import CutPool  # LG branch-and-cut cut pool
from .queues import NodeQueue, LIFONodeQueue, FIFONodeQueue, WorstBoundNodeQueue, QueueStrategy, WorstBoundkSolutions
...
_queue_map[QueueStrategy.bound_k_solutions] = WorstBoundkSolutions
...
class Tree():
    ...
    def __init__(
        ...
        self.selection_strategy = params._selection_strategy(id_to_var = subproblems.id_to_vars,
                                                             ...
                                                             max_simplex_iterations = params._max_simplex_iterations,
                                                             hybrid_starting_method = params._hybrid_starting_method,
                                                             hybrid_switch_iter = params._hybrid_switch_iter,
                                                             subset_branching_variables = params._subset_branching_variables)
        self.parition_strategy  = params._partition_strategy()
    
        # LG branch-and-cut: global cut pool for Lagrangean cuts
        self.cut_pool = CutPool(max_cuts_per_scenario=100)
    
        # need a root node
        self.init_root_node(subproblems)

        # convergence metrics - updated at solve call
        self.max_iter = 100  # FIX: removed trailing comma (was tuple)
        self.rel_tolerance = 1e-2  # FIX: removed trailing comma
        self.abs_tolerance = 1e-3  # FIX: removed trailing comma
        self.time_limit = math.inf  # FIX: removed trailing comma
    ...
    def init_root_node(self, 
        ...
        self.node_queue.push(root, None, math.inf)
    ...
    def branch(self, 
        ...
        self.node_queue.push(left_child_node, current_node.ub_problem.candidate, current_node.ub_problem.objective)
        self.node_queue.push(right_child_node, current_node.ub_problem.candidate, current_node.ub_problem.objective)

        self.metrics.nodes.node_id_max += 2
        del current_node

        # lowerbound update
        return self.update_lb()  # FIX: restored return statement
    

    def _spawn_binary_children(self, 
                               current_node: Node,
                               var_name: str,
                               *ars, **kwargs) -> Tuple[Node, Node]:
        """
        ...
        # LG STATE PROPAGATION (guarded: parent may not have lg_multipliers)
        parent_mu = getattr(current_node, 'lg_multipliers', None)
        left_child_node.lg_multipliers = cp.deepcopy(parent_mu) if parent_mu else {}
        right_child_node.lg_multipliers = cp.deepcopy(parent_mu) if parent_mu else {}
    ...
    def update_ub(self, 
        ...
        ---------------
        ub_objective : float
            new upper bound objective to save.
        """
    ...
    def update_lb(self) -> str:
        ...
        Iterates over each of the open leaf nodes, saves the lowest LB.
    ...
    def n_nodes(self) -> int:
        """
        How many nodes are currently in the tree?
