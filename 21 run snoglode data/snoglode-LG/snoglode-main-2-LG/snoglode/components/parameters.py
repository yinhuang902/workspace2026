...
from snoglode.components.branching import SelectionStrategy, PartitionStrategy, RandomSelection, Midpoint, MaximumDisagreement
...
class SolverParameters():
    def __init__(self, 
        ...
        subproblem_creator: callable
            ...
            and the list of complicating variables.
        ...
        # FIX: corrected type check (was comparing bool instead of type)
        assert isinstance(subproblem_names, (int, list)), \
        ...
        self.guarantee_global_convergence()
        self.set_rank_subproblem_names(list())
        self.set_epsilon(1e-3)
        self.set_bounds_tightening()
        self.relax_binaries(False)
        self.relax_integers(False)
        # bounder problem information
        self.set_bounders()
        self.set_queue_strategy()
        self.add_node_feasibility_checker(None)
    ...
    def set_bounds_tightening(self, 
        ...
        assert type(obbt_solver_opt) == dict or obbt_solver_opt is None
    ...
    def set_bounders(self,
                     lower_bounder: AbstractLowerBounder = LGLowerBounder,
    ...
    def set_queue_strategy(self, 
                           strategy: QueueStrategy = QueueStrategy.bound) -> None:
        """
    ...
    def add_node_feasibility_checker(self,
        ...
        """
        assert callable(feasibility_checker) or feasibility_checker is None
    ...
    def set_branching(self,
                      ...
                      partition_strategy: PartitionStrategy = Midpoint,
                      max_simplex_iterations: int = 1000,
                      subset_branching_variables: int = None,
                      hybrid_starting_method: SelectionStrategy = MaximumDisagreement,
                      hybrid_switch_iter: int = 10) -> None:
        """
        ...
        max_simplex_iterations : int, optional
            only for strong branching strategies!
            indicates how many iterations of primal simplex to perform
            before terminating the strong branching solve. By default, 1000.
        subset_branching_variables : int, optional
            only for strong branching strategies!
            indicates how many of the branching variables to consider
            for strong branching. By default, None, which means all.
        hybrid_starting_method : SelectionStrategy, optional
            only for hybrid branching!
            indicates what method to use for the first n iteration
            of branching. By default, MaximumDisagreement.
        hybrid_switch_iter : int, optional
            only for hybrid branching!
            indicates after how many iterations we switch from the
            hybrid_starting_method to Pseudocost.
        """
        self._selection_strategy = selection_strategy
        self._selection_strategy_solver = selection_strategy_solver
        self._partition_strategy = partition_strategy
        self._max_simplex_iterations = max_simplex_iterations
        self._subset_branching_variables = subset_branching_variables
        self._hybrid_starting_method = hybrid_starting_method
        self._hybrid_switch_iter = hybrid_switch_iter
    ...
    def display(self) -> None:
        ...
        if type(self._lb_solver) == pyomo.solvers.plugins.solvers.GAMS.GAMSShell:
            print(f"\nLB solver: using Gams Shell, {self._lb_solver.options}" )
        else: print(f"\nLB solver: {self._lb_solver.name}" )
        if type(self._ub_solver) == pyomo.solvers.plugins.solvers.GAMS.GAMSShell:
            print(f"UB solver: using Gams Shell, {self._ub_solver.options}")
        else: print(f"UB solver: {self._ub_solver.name}" )
        if self._cg_solver == None: print(f"CG solver: None")
        elif type(self._cg_solver) == pyomo.solvers.plugins.solvers.GAMS.GAMSShell:
            print(f"CG solver: using Gams Shell, {self._cg_solver.options}")
        ...
        if self._selection_strategy.__name__ is "StrongBranching":
            print(f"      -> Max Num. Simplex Iterations: {self._max_simplex_iterations}")
            print(f"      -> Subset of Branching Variables: {self._subset_branching_variables}")
        if self._selection_strategy.__name__ is "FullStrongBranching":
            print(f"      -> Max Num. Simplex Iterations: {self._max_simplex_iterations}")
