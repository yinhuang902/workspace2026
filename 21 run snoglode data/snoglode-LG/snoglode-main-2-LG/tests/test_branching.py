...
def test_binary_branching_lifo(integer_program, mock_cg):
    """
    ...
    params.set_bounders(candidate_solution_finder = mock_cg)
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_binary_branching_fifo(integer_program, mock_cg):
    """
    If we have binary branching, check that each node is generated properly.
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_binary_branching_bound(integer_program, mock_cg):
    """
    If we have binary branching, check that each node is generated properly.
...
def test_binary_tree_terminal_leaf_nodes(pmedian, mock_cg):
    """
    ...
    params = sno.SolverParameters(subproblem_names = subproblem_names,
                                  subproblem_creator = pmedian,
                                  cg_solver = pyo.SolverFactory("gurobi"))
    params.set_bounds_tightening(fbbt = False,
                                 obbt = False)
    params.activate_verbose()
    params.inherit_solutions_from_parent(False)
    params.set_queue_strategy(sno.QueueStrategy.bound)
    params.set_bounders(candidate_solution_finder = mock_cg)
    solver = sno.Solver(params)
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_continous_branching_epsilon_tolerance(continuous_1var, mock_cg):
    """
    ...
    np.random.seed(2)
    params = sno.SolverParameters(subproblem_names=["dummy"],
                                  subproblem_creator=continuous_1var,
                                  cg_solver = pyo.SolverFactory("gurobi"))
    params.set_bounds_tightening(fbbt = False,
                                 obbt = False)
    params.activate_verbose()
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_relaxed_binary_branching(integer_program):
    """
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_most_infeasible_binary_branching_strategy(integer_program):
    """
    When we choose to relax binaries, check that it will progress
    as expected.
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_maximum_disagreement_branching_strategy(farmer_classic, gurobi):
    """
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_pseudocost_branching_strategy(farmer_classic, gurobi):
    """
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_integer_branching(integer_knapsack):
    """
    ...
    """
    params = sno.SolverParameters(subproblem_names = ["dummy"],
                                subproblem_creator = integer_knapsack)
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_full_strong_branching_ip(integer_knapsack):
    """
    Test that full strong branching works as expected - integer variables.
    Classic use case: linear relaxations at the lower bound.
    """
    params = sno.SolverParameters(subproblem_names = ["dummy"],
                                  subproblem_creator = integer_knapsack)
    params.set_bounds_tightening(fbbt = False, obbt=False)
...
@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
def test_full_strong_branching_binaries(integer_program):
    """
    Test that full strong branching works as expected - binary variables.
    Classic use case: linear relaxations at the lower bound.
    """
    # reset seed 
    np.random.seed(2)
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_full_strong_branching_lp(farmer_classic):
    """
    Test that full strong branching works as expected - linear program.
    Classic use case: linear relaxations at the lower bound.
    """
    # reset seed 
    np.random.seed(17)
    ...
    assert solver.tree.metrics.relative_gap <= 0.1 # should converge based on meeting relative gap
    assert solver.tree.metrics.lb == pytest.approx(-109422.5, rel=1e-2)
    assert solver.tree.metrics.ub == pytest.approx(-108349.77, rel=1e-2)
    assert solver.iteration == 30 # should converge in 30 iterations or less
    assert solver.runtime <= 5 # should run in under 1 second


@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_strong_branching_lp(farmer_classic):
    """
    Test that full strong branching works as expected - linear program.
    Classic use case: linear relaxations at the lower bound.
    """
    ...
    params.activate_verbose()
    params.deactivate_global_guarantee()
    params.set_branching(selection_strategy = sno.StrongBranching,
                         max_simplex_iterations = 10,
                         subset_branching_variables = 1,
                         partition_strategy = sno.ExpectedValue)
    
    solver = sno.Solver(params)
    solver.solve(max_iter=30) # full strong branching only takes 30 iterations to converge; we want to compare

    assert solver.tree.metrics.lb == pytest.approx(-110646.5, rel=1e-2)
    assert solver.tree.metrics.ub == pytest.approx(-108095.6, rel=1e-2)
    assert solver.runtime <= 5 # should run in under 0.5 seconds


@pytest.mark.requires_gurobi
@pytest.mark.mpi_skip()
@pytest.mark.xfail(raises=AssertionError)
def test_full_strong_branching_nlp(quadradic):
    """
    Test that full strong branching raises an error if we try to use it
    on a non-linear problem - can have integers/binaries, but not 
    nonlinear constraints
    """
    params = sno.SolverParameters(subproblem_names = ["scenario1"],
                                  subproblem_creator = quadradic)
    params.set_bounds_tightening(fbbt = False, obbt = False)
    params.set_bounders(candidate_solution_finder = sno.AverageLowerBoundSolution,
                        lower_bounder = sno.DropNonants)
    params.set_branching(selection_strategy = sno.FullStrongBranching,
