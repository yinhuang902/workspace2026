...
def test_farmer_classic_ef_bounds_tightening(farmer_classic, gurobi):
    """
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_farmer_classic_average_solution_bounds_tightening_fbbt(farmer_classic):
    """
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_pmedian_averge_solution_fbbt(pmedian):
    """
    Here, we solve using the averaging of the lower bound solution and following specifications
    with FBBT and without, asserting everything remains the same.
    The only thing that can be variable here should be the time of the algorithm progression.
...
@pytest.mark.requires_gurobi
@pytest.mark.skipif(size > 3, reason="test can run with at most 3 ranks.")
def test_pmedian_averge_solution_obbt(pmedian):
    """
