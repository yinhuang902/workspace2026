...
class GurobiLBLowerBounder(sno.AbstractLowerBounder):
    def __init__(self, 
        ...
        super().__init__(solver = solver, inherit_solutions = inherit_solutions)
        assert "gurobi" in solver.name
        self.iter = 0
    ...
    def solve_a_subproblem(self, 
        ...
        self.iter += 1
        # this ensures we are updating the MIP gap starting at iter 1 after every LB solve
        # otw, we will tighten for each s \in S individually, which is not the intention
        if self.iter % num_scenarios == 0 and self.current_milp_gap != 1e-4 \
