    ...
    def solve(self, 
        ...
        for k in range(self.K):
            ...
            if rank == 0:
                ...
                print(f"LG DIAG [k={k}]: ||mu||={mu_norm:.4f}, max|mu|={mu_max:.4f}, step={step_size:.4f}")
                ...
                # Calculate y_bar (Consensus) from scenarios with primal solutions
                ...
                # Subgradient update with step size t_k = t0 / sqrt(k+1)
                # Only update for scenarios with primal solutions
                for name in scenarios_with_primal:
                    dual_bound, y_vals = global_results[name]
                    if y_vals:
                        for vid in all_var_ids:
        ...
        if rank == 0:
            ...
            if hasattr(self, 'cut_pool') and self.cut_pool is not None:
                ...
                # DEBUG: Print cut statistics
                print(f"DEBUG RMP: Node {node.id}, new_cuts={len(new_cuts)}, pool_cuts={len(pool_cuts)}, total={len(all_cuts)}, pool_size={len(self.cut_pool)}")
