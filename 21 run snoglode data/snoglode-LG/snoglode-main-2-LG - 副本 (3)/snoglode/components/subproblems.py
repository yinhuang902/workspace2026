...
class Subproblems():
    ...
    def __init__(self, 
        ...
        subproblem along with identification of the complicating variables.
        ...
        subproblem_creator : function
            given subproblem number, returns pyomo model, dictonary of key pairs 
            (varID, pyo.Var) and the subproblem probability
        use_fbbt : bool
        ...
        self.subproblem_complicating_vars = {subproblem_name: [] for subproblem_name in self.names}
        ...
        # keep track of the # of complicating vars in each generated scenario --> sanity checks
        for subproblem_name in self.names:

            # generate scenario model + list of first stage variables
            subproblem_model, subproblem_complicating_variables, subproblem_probability \
                    = subproblem_creator(subproblem_name)

            # check we have the right types
            assert type(subproblem_complicating_variables)==dict, \
                "second return of subproblem_creator must be a dict of (complicating var id) : pyo.Var."
            assert len(subproblem_complicating_variables.keys())!=0, \
                "must be at least one complicating var element."
            ...
            assert get_active_objective(subproblem_model).sense == pyo.minimize, \
                "SNoGloDe currently only supports minimization problems.\n" + \
                "  Hint: max(obj) := min(-obj)"
            
            if self.verbose: print(f"\tScenario {subproblem_name} has {len(subproblem_complicating_variables.keys())} first stage vars.")
            ...
            if self.use_obbt: self._perform_obbt(model = subproblem_model,
                                                 variables = self.subproblem_complicating_vars[subproblem_name])

            # INITIALIZING THE COMPLICATING VARIABLES -> for branching
            for complicating_var_id in subproblem_complicating_variables:

                # update information in the dict.
                self._init_complicating_var(node_state = root_node_state,
                                      complicating_var_id = complicating_var_id,
                                      complicating_var = subproblem_complicating_variables[complicating_var_id],
            ...
            # ========================================================================================= #
            # LG MODIFICATION: Add Lagrange multipliers and modify objective
            # ========================================================================================= #
            # We add a mutable Param for each complicating variable to represent mu
            # And update the objective to: f(x,y) - mu^T y
            
            # 1. Create the mutable Param indexed by the complicating variable IDs
            #    We use the IDs as keys for the Param
            subproblem_model.lg_mu = pyo.Param(subproblem_complicating_variables.keys(), 
                                               initialize=0.0, 
                                               mutable=True)
            
            # 2. Get the current active objective
            current_objective = get_active_objective(subproblem_model)
            if current_objective is None:
                raise RuntimeError(f"Subproblem {subproblem_name} has no active objective!")
            
            # 3. Define the Lagrangian term: sum(mu_i * y_i)
            #    Note: subproblem_complicating_variables map ID -> Var
            lagrangian_term = sum(subproblem_model.lg_mu[var_id] * subproblem_complicating_variables[var_id] 
                                  for var_id in subproblem_complicating_variables)
            
            # 4. Create new objective expression
            #    We subtract the term bc we are MINIMIZING ( min f(x) - mu^T y )
            #    Standard LG for minimization: L(y, mu) = f(y) + mu^T(z - y) ... wait.
            #    User specified: "Modify the scenario objective from f_ω(...) to: f_ω(...) − μ_ω^T y_ω"
            #    So we strictly follow that.
            new_expr = current_objective.expr - lagrangian_term
            
            # 5. Deactivate old objective and add new one
            current_objective.deactivate()
            subproblem_model.lg_objective = pyo.Objective(expr=new_expr, sense=pyo.minimize)
            # ========================================================================================= #
        ...
        # accumulate all of the complicating variables dictionary across ranks
        self._sync_all_complicating_vars(root_node_state)
        ...
        if self.relax_binaries and len(self.root_node_state[SupportedVars.binary]): 
            self.binary_var_ids = [complicating_var for complicating_var in self.root_node_state[SupportedVars.binary]]
        
        # if we relax integers, save a list of those var ID's
        if self.relax_integers:
            if self.root_node_state[SupportedVars.integers]: 
                self.integer_var_ids = [complicating_var for complicating_var in self.root_node_state[SupportedVars.integers]]
            if self.root_node_state[SupportedVars.nonnegative_integers]:
                self.nonneg_integer_var_ids = [complicating_var for complicating_var in self.root_node_state[SupportedVars.nonnegative_integers]]
        
        # extract the list of all variable ids
        self.complicating_var_ids = [complicating_var for var_domain in SupportedVars\
                                    for complicating_var in self.root_node_state[var_domain]]
    ...
    def set_subproblem_state(self, 
        ...
        set_second_stage: bool
            ...
            variables to their bounds, or not.
        """
        # for each of the complicating variables
        for complicating_var in self.subproblem_complicating_vars[subproblem_name]:

            # extract complicating var ID, domain
            var_type, complicating_var_id, subproblem_name = self.var_to_data[complicating_var]

            # update variables state to reflect node
            self._update_complicating_var_state(subproblem_name = subproblem_name,
                                          complicating_var = complicating_var,
                                          complicating_var_state = state[var_type][complicating_var_id])
    ...
    def tighten_and_sync_bounds(self, 
                                node: Node) -> None:
        """
        ...
        # update the node
        self._sync_all_complicating_vars(node.state)
    ...
    def relax_all_binaries(self) -> None:
        ...
        """
        if not hasattr(self, "binary_var_ids"): return
    ...
    def relax_all_integers(self) -> None:
        ...
        """
        if not hasattr(self, "integer_var_ids") and \
            not hasattr(self, "nonneg_integer_var_ids"): return
    ...
    def save_results_to_dict(self) -> dict:
        ...
        -------
        results : dict
            format is results[scenario name][variable name] -> variable value
        """
    ...
    def _init_complicating_var(self, 
                         node_state: dict,
                         complicating_var_id: str, 
                         complicating_var: pyo.Var,
                         subproblem_name: str) -> None:
        """
        Given a pyomo variable (identified as complicating)
        ...
        -----------
        node_state : dict
            dictionary corresponding to the node state we are building.
        complicating_var_id : hashable type
            complicating pyomo variable id value.
        complicating_var : pyo.Var
            complicating pyomo variable.
        subproblem_name : str
            subproblem string name representation corresponding to this subproblem.
        """

        # should not have any indexed vars...
        assert not complicating_var.is_indexed(), \
            f"{complicating_var.name} was passed as an indexed Var. \n \
                Every complicating variable should be passed as a single element, not indexed. "
        
        # to use branching, we need to have bounds on all complicating vars
        # SAFETY PATCH: Also require FINITE bounds for LG lower bounding (RMP must be bounded)
        assert complicating_var.has_lb() and complicating_var.has_ub(), \
            f"{complicating_var.name} was passed without bounds. \n \
                Every complicating variable must have bounds to perform branching. "
        
        import math
        assert math.isfinite(complicating_var.lb) and math.isfinite(complicating_var.ub), \
            f"{complicating_var.name} has non-finite bounds (lb={complicating_var.lb}, ub={complicating_var.ub}). \n \
                Complicating variables must have finite bounds for LG lower bounding."

        # extract domain and convert to associated support variable domain
        var_type = SupportedVars(complicating_var.domain.local_name)
        
        # add var to ComponentMap
        self.var_to_data[complicating_var] = (var_type, complicating_var_id, subproblem_name)

        # if we have not entered this variable already, add it now
        if complicating_var_id not in self.id_to_vars:

            # add to ID -> [vars]
            self.id_to_vars[complicating_var_id] = [complicating_var]

            # add to the node state
            node_state[var_type][complicating_var_id] = \
                        ComplicatingVariable(domain = var_type,
                                       lb = complicating_var.lb,
                                       ub = complicating_var.ub,
                                       var_id = complicating_var_id)
            
        # if we have already entered this variable, update it.
        else:
            # add to ID -> [vars]
            self.id_to_vars[complicating_var_id].append(complicating_var)

            # update on the node state (saves best bounds)
            node_state[var_type][complicating_var_id].update(complicating_var)
            
        # add to list of complicating variables for this subproblem
        self.subproblem_complicating_vars[subproblem_name].append(complicating_var)
    ...
    def _sync_all_complicating_vars(self,
                              node_state: dict) -> None:
        """
        After we have initialized all of the complicating variables across 
        ...
        different bounds on the same complicating variable. Make sure this is
        ...
        if (size > 1):
            ...
            # based on how we stored the complicating vars, go through each of the types and
            # gather all of the complicating_var_ids on each rank associated with this
            for var_domain in SupportedVars:

                # determine the number of variables we have for this domain
                num_complicating_vars = len(node_state[var_domain])
                total_num_complicating_vars = MPI.COMM_WORLD.allreduce(num_complicating_vars, op=MPI.SUM)

                # only perform sync operations if there exists complicating variables for this domain
                if (total_num_complicating_vars > 0):

                    # gather all names of complicating variables from all ranks
                    complicating_vars = list(complicating_var_id for complicating_var_id in node_state[var_domain])
                    all_complicating_vars = MPI.COMM_WORLD.allgather(complicating_vars)

                    # flatten list (itertools) & take the set to remove duplicates
                    set_of_complicating_vars = sorted(set(itertools.chain.from_iterable(all_complicating_vars)))

                    # sync all names / bounds on the current dict
                    for complicating_var_id in set_of_complicating_vars:

                        # if this rank does not have it, add
                        if complicating_var_id not in node_state[var_domain]:
                            node_state[var_domain][complicating_var_id] = \
                                ComplicatingVariable(domain = var_domain,
                                               lb = -math.inf,
                                               ub = math.inf,
                                               var_id = "NAN")
                        ...
                        best_lb = MPI.COMM_WORLD.allreduce(node_state[var_domain][complicating_var_id].lb, op=MPI.MAX)
                        best_ub = MPI.COMM_WORLD.allreduce(node_state[var_domain][complicating_var_id].ub, op=MPI.MIN)

                        # update to best bounds
                        node_state[var_domain][complicating_var_id].lb = best_lb
                        node_state[var_domain][complicating_var_id].ub = best_ub


    def _update_complicating_var_state(self,
                                 subproblem_name: str,
                                 complicating_var: pyo.Var, 
                                 complicating_var_state: dict) -> None:
        """
        ...
        complicating_var_state : dict
            information reflecting the variables current state
            that we are to update to.
        """
        # retrieve fixed state status & update
        if complicating_var_state.is_fixed: complicating_var.fix(complicating_var_state.value)
        else: complicating_var.unfix()

        # update bounds
        assert complicating_var_state.lb <= complicating_var_state.ub
        complicating_var.lb = complicating_var_state.lb
        complicating_var.ub = complicating_var_state.ub
    ...
    def _perform_fbbt(self, 
        ...
        model : pyo.ConcreteModel
            Model that we aim to perform FBBT on.
        
        Returns
        --------
        variable_ranges : pyo.ComponentMap
            mapping of pyo.Var to (lb, ub) after FBBT.
        """
    ...
    def _perform_obbt(self, 
        ...
        --------
        variable_ranges : pyo.ComponentMap
            mapping of pyo.Var to (lb, ub) after OBBT.
        """ 
        try:
    ...
    def _tighten_rank_subproblem_bounds(self,
        ...
        for subproblem_name in self.names:
            ...
            # for each of the complicating variables within this subproblem
            variables = []
            for var in self.subproblem_complicating_vars[subproblem_name]:
            ...
            for var in variables:
                ...
                else: lb_fbbt, ub_fbbt = fbbt_bounds[var]
                
                if (not self.use_obbt) or (obbt_bounds == False):
                    lb_obbt, ub_obbt = max_bounds
                else: lb_obbt, ub_obbt = obbt_bounds[var]
    ...
    def _check_bounds(self, 
        ...
        -------
        feasible : bool
            if the bounds are feasible, return True,
    ...
    def _convert_to_nonneg_integer(self, 
                                   var: pyo.Var) -> None:
        """
        Convert domain from pyo.Reals -> pyo.Binary
...
class ComplicatingVariable():
    ...
    def __init__(self,
        ...
        """
        creates a new complicating variable object.
    ...
    def update(self, 
               var: pyo.Var) -> None:
        """
        updates a currently instantiated complicating variable
...
class DummyComplicatingVariable():
    """
    When we do not have a complicating variable present on a current
    ...
    def __init__(self,
                 ...
                 ub: float) -> None:
        """
        Initializes a dummy complicating variable.
    ...
    def update(self, *args, **kwargs) -> None:
        raise NameError("The DummyComplicatingVariable cannot be updated.")