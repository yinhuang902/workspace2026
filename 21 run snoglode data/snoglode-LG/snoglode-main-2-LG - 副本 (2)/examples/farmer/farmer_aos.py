...
if __name__=="__main__":
    ...
    if (size==1): params.set_logging(fname = os.getcwd() + "/logs/farmer_classic_log")
    else: params.set_logging(fname = os.getcwd() + "/logs/farmer_classic_log_parallel")
    params.set_bounds_tightening(False, False)
    params.set_bounders(candidate_solution_finder = sno.AverageLowerBoundSolution)
    params.set_queue_strategy(sno.QueueStrategy.bound_k_solutions)
    if (rank==0): params.display()

    solver = sno.Solver(params)
    solver.solve(max_iter=100)

    solutions = solver.tree.node_queue.solution_pool
    solution_dict = solutions.to_dict()
    pprint(solution_dict)
    quit()
