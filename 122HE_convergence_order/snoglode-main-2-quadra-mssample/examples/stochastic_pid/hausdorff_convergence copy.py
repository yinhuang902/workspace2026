...
class GurobiLBLowerBounder(sno.AbstractLowerBounder):
    ...
    def solve_a_subproblem(self, subproblem_model, *args, **kwargs):
        ...
        if results.solver.termination_condition == TerminationCondition.maxTimeLimit:
            if ipopt.available():
                try:
                    # IPOPT with load_solutions=True loads directly into model
                    ipopt.solve(subproblem_model, load_solutions=True)
                    used_ipopt_fallback = True
                    # Return objective value directly from model
        ...
        if results.solver.termination_condition == TerminationCondition.optimal and results.solver.status == SolverStatus.ok:
            # Load Gurobi solutions
            subproblem_model.solutions.load_from(results)
            
            # Use lower_bound only if available (Gurobi provides this)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Estimate Hausdorff convergence order in decision space.")
    ...
    for k in range(args.k_min, args.k_max + 1):
        ...
        # Compute LB (kept for completeness)
        ...
        LB_val = node.lb_problem.objective
        print(f"  LB: {LB_val} (time: {lb_time:.2f}s)")
