    ...
    def update_lb(self) -> str:
        ...
        if (lb != self.metrics.lb):
            ...
            if deviation > tolerance: 
                print("WARNING: LB oscillation detected (ignored).")
                print(f" prev LB: {self.metrics.lb}, attempted new LB: {lb}")
            self.metrics.lb = max(self.metrics.lb, lb)  # Enforce monotonicity
            return "lb"
        else: return ""
