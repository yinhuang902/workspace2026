...
function scenario(S,NS)
    ...
    @constraint(m, eqdyn[s in S,t in Tm], (1/tau[s])*(x[s,t+1]-x[s,t])/h + x[s,t+1] == K[s]*u[s,t+1]+Kd[s]*d[s,t])
    ...
    @constraint(m, costdef[s in S,t in mT], cost[s,t] == (10*(x[s,t]-xsp[s])^2 + 0.01*u[s,t]^2))

    @variable(m, costS[s = S])
    @constraint(m, costSdef[s in S], costS[s] == 100/length(T)/NS * sum{cost[s,t], t=T})
