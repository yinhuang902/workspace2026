...
from utils import (
    MIN_DIST, ACTIVE_TOL, MS_AGG, MS_CACHE_ENABLE, GAP_STOP_TOL, UB_REF_CAP,
...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    while len(nodes) < target_nodes:
        ...
        # --- Cap UB_global by reference ub_ref_cap (for gap/active-simplex stabilization) ---
        # Use config UB_REF_CAP from utils.py if ub_ref_cap not explicitly passed
        effective_cap = ub_ref_cap if ub_ref_cap is not None else UB_REF_CAP
        if effective_cap is not None:
            UB_global = min(UB_global, float(effective_cap))
