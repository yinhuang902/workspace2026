...
class MSBundle:
    ...
    def update_tetra(self, tet_vertices, fverts_scene):
        ...
        # Critical for Gurobi's McCormick relaxation of bilinear terms
        # (K_p*e_s, K_i*I, K_d*dedt). Without this, the relaxation stays at
        # full-domain width even on tiny child simplices, preventing LB improvement.
        self.Kp.setlb(min(vx))
        self.Kp.setub(max(vx))
        self.gp.update_var(self.Kp)

        self.Ki.setlb(min(vy))
        self.Ki.setub(max(vy))
        self.gp.update_var(self.Ki)

        self.Kd.setlb(min(vz))
        self.Kd.setub(max(vz))
        self.gp.update_var(self.Kd)
