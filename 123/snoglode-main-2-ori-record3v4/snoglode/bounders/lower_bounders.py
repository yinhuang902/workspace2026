...
def safely_get_lower_bound(results):
    try:
...
class DropNonants(AbstractLowerBounder):
    ...
    def solve_a_subproblem(self, 
        ...
        # Robust handling for non-optimal terminations (maxTimeLimit, unbounded, etc.)
        else:
            term_cond = results.solver.termination_condition
            logging.warning(f"Subproblem terminated with condition: {term_cond}")

            # Attempt to load solution if available (for side-effects/warm-start ONLY, not for LB value)
            try:
                subproblem_model.solutions.load_from(results)
            except Exception:
                pass 

            # Logic to find a VALID Lower Bound (Dual Bound)
            lb_val = None
            
            # Priority 1: Solver's reported lower bound
            lb_val = safely_get_lower_bound(results)
            ...
            if lb_val is None:
                lb_val = LB_FLOOR
                logging.warning(f"No valid LB found for {term_cond}. Using conservative LB_FLOOR: {LB_FLOOR}")
