...
# Conservative floor for lower bound when no valid bound is available
LB_FLOOR = -1e10

def _is_finite_bound(value) -> bool:
    """Check if a bound value is finite and usable."""
    import math
    if value is None:
        return False
    try:
...
class DropNonants(AbstractLowerBounder):
    ...
    def solve_a_subproblem(self, 
        ...
        # Handle non-optimal terminations (maxTimeLimit, unbounded, infeasibleOrUnbounded, etc.)
        # IMPORTANT: Do NOT return (False, None) as that marks the node as infeasible!
        else:
            termination = results.solver.termination_condition
            model_name = getattr(subproblem_model, 'name', 'unknown')
            
            # Try to load incumbent solution for side effects (warm-start, variable values)
            try:
                if hasattr(results, 'solution') and len(results.solution) > 0:
                    subproblem_model.solutions.load_from(results)
            except Exception:
                pass  # Incumbent loading is optional; failure is acceptable
            
            # Priority for valid lower bound:
            # (a) results.problem.lower_bound if finite
            # (b) successor_obj if finite (valid because successor_lb_cut enforces obj >= successor_obj)
            # (c) LB_FLOOR as last resort
            lb_value = None
            lb_source = None
            
            # (a) Try dual bound from solver
            try:
                dual_bound = results.problem.lower_bound
                if _is_finite_bound(dual_bound):
                    lb_value = float(dual_bound)
                    lb_source = "dual_bound"
            except (AttributeError, TypeError):
                pass
            
            # (b) Try successor_obj (valid LB due to successor_lb_cut constraint)
            if lb_value is None:
                try:
                    successor_obj = pyo.value(subproblem_model.successor_obj)
                    if _is_finite_bound(successor_obj):
                        lb_value = float(successor_obj)
                        lb_source = "successor_obj"
                except (AttributeError, TypeError, ValueError):
                    pass
            
            # (c) Last resort: conservative floor
            if lb_value is None:
                lb_value = LB_FLOOR
                lb_source = "LB_FLOOR"
                logging.getLogger('snoglode.bounders').warning(
                    f"WARNING: Non-optimal termination '{termination}' for subproblem '{model_name}' "
                    f"with no valid bound available. Using conservative LB_FLOOR={LB_FLOOR}."
                )
            else:
                logging.getLogger('snoglode.bounders').debug(
                    f"DEBUG: Non-optimal termination '{termination}' for subproblem: {model_name}. "
                    f"Using {lb_source}={lb_value} as lower bound."
                )
