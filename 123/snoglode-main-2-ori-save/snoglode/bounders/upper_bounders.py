...
class UpperBounder(BoundingProblemBase):
    ...
    def solve_subproblem(self, 
        ...
        scenario_objective : float
            The value of the objective; returns None if infeasible, inf if no incumbent.
        """
        # solve model
        ...
        # if the solution is infeasible, this candidate is infeasible
        elif results.solver.termination_condition == TerminationCondition.infeasible:
            return False, None
        
        # Handle non-optimal terminations (maxTimeLimit, unbounded, infeasibleOrUnbounded, etc.)
        # IMPORTANT: Do NOT return (False, None) for non-infeasible cases as that marks the node as infeasible!
        else:
            termination = results.solver.termination_condition
            model_name = getattr(subproblem_model, 'name', 'unknown')
            
            # Try to load incumbent solution and get objective
            try:
                if hasattr(results, 'solution') and len(results.solution) > 0:
                    subproblem_model.solutions.load_from(results)
                    obj_value = pyo.value(get_active_objective(subproblem_model))
                    # Check if objective is finite
                    if obj_value is not None and not math.isnan(obj_value) and not math.isinf(obj_value):
                        logging.getLogger('snoglode.bounders').debug(
                            f"DEBUG: Non-optimal termination '{termination}' for UB subproblem: {model_name}. "
                            f"Using incumbent objective={obj_value}."
                        )
                        return True, obj_value
            except Exception:
                pass  # Failed to load incumbent or evaluate objective
            
            # No incumbent available - return +inf as a conservative upper bound
            logging.getLogger('snoglode.bounders').warning(
                f"WARNING: Non-optimal termination '{termination}' for UB subproblem '{model_name}' "
                f"with no usable incumbent. Returning +inf as upper bound."
            )
