    ...
    def solve_a_subproblem(self, 
        ...
        if results.solver.termination_condition==TerminationCondition.maxTimeLimit:
            try:
                ...
                print(f"WARNING: Ipopt fallback crashed on subproblem '{subproblem_name}'. Using Gurobi bound. Error: {e}")
                parent_obj = pyo.value(subproblem_model.successor_obj)
                gurobi_lb = results.problem.lower_bound if results.problem.lower_bound is not None else parent_obj
                return True, max(parent_obj, gurobi_lb)
