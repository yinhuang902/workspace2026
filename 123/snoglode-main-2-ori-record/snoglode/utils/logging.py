...
class IterLogger():
    ...
    def __init__(self, 
        ...
        self.csv_file = None
        if self.csv_log_name is not None:
            self.csv_file = open(self.csv_log_name + ".csv", "w")
            # Write header
            header = ["Iteration", "Time", "LB", "UB", "Rel_Gap", "Abs_Gap", "Nodes_Explored", "Open_Nodes"]
    ...
    def update(self, tree=None):
        ...
        if (rank == 0) and (self.csv_file is not None) and (tree is not None):
            row = [
                self.iter,
                round(time.perf_counter() - self.snoglode_start, 4),
                tree.metrics.lb,
                tree.metrics.ub,
                tree.metrics.relative_gap,
                tree.metrics.absolute_gap,
                tree.metrics.nodes.explored,
                tree.n_nodes()
            ]
            self.csv_file.write(",".join(map(str, row)) + "\n")
            self.csv_file.flush()
...
class MockIterLogger():
    ...
    def update(self, tree=None): pass
