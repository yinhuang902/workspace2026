...
class Solver():
            ...
            try:
                ...
                if len(iterations) > 0:
                    ...
                    ax.plot(iterations, self.wlsq_hist["original_lb"], 
                            marker='D', markersize=4, label='Original Node LB', 
                    ...
                    ax.set_title('Lower Bound Comparison (Per-Node)', fontsize=14, fontweight='bold')
                    # ax.set_ylim(SUMMARY_PLOT_LB_YMIN, SUMMARY_PLOT_LB_YMAX) # Auto-scale
    ...
    def dispatch_node_solver(self, 
        ...
        # --- WLSQ Surrogate OBBT (Optional Bounds Tightening) ---
        # Initialize diagnostic info for this iteration
        current_node._wlsq_obbt_diag = {
            'status': 'OFF',
            'shrink_pct': None,
            'ratio': None,
            'tightened_count': 0
        ...
        if (self._params._wlsq_obbt and 
            self.iteration % self._params._wlsq_obbt_frequency == 0):
            
            # Compute UB for tightening: min of global UB and current node's UB
            import math
            ub_for_tightening = self.tree.metrics.ub
            if hasattr(current_node.ub_problem, 'objective') and \
               current_node.ub_problem.objective is not None and \
               math.isfinite(current_node.ub_problem.objective):
                ub_for_tightening = min(ub_for_tightening, current_node.ub_problem.objective)
            
            # Synchronize UB across all ranks before tightening
            ub_for_tightening = MPI.COMM_WORLD.allreduce(ub_for_tightening, op=MPI.MIN)
            
            # Check UB
            if not math.isfinite(ub_for_tightening):
                current_node._wlsq_obbt_diag['status'] = 'NOUB'
            # Check surrogate
            elif not hasattr(current_node.lb_problem, 'wlsq_surrogate') or \
                 current_node.lb_problem.wlsq_surrogate is None:
                current_node._wlsq_obbt_diag['status'] = 'NOSURR'
            else:
                # Compute V_before
                V_before = 1.0
                if SupportedVars.reals in current_node.state:
                    for vid, vdata in current_node.state[SupportedVars.reals].items():
                        width = max(vdata.ub - vdata.lb, 0.0)
                        V_before *= width
                
                from snoglode.utils.wlsq_obbt import tighten_node_box_by_wlsq_surrogate
                
                result = tighten_node_box_by_wlsq_surrogate(
                    node=current_node,
                    subproblems=self.subproblems,
                    ub_value=ub_for_tightening,
                    delta=self._params._wlsq_obbt_delta,
                    epsilon=self.tree.epsilon,
                    only_convex=self._params._wlsq_obbt_only_convex,
                    solver_name=self._params._wlsq_obbt_solver_name,
                    solver_opts=self._params._wlsq_obbt_solver_opts
                )
                ...
                if result.get('infeasible', False):
                    status = 'INF'
                    current_node.lb_problem.is_infeasible()
                    current_node.ub_problem.is_infeasible()
                elif result.get('skipped_nonconvex', False):
                    status = 'SKIPNC'
                else:
                    status = 'OK'
                    # Apply tightened bounds
                    if result.get('tightened_count', 0) > 0:
                        self.subproblems.set_all_states(current_node.state, set_second_stage=False)
                
                current_node._wlsq_obbt_diag = {
                    'status': status,
                    'shrink_pct': shrink_pct,
                    'ratio': ratio,
                    'tightened_count': result.get('tightened_count', 0)
                }
    ...
    def dispatch_updates(self,
        ...
        # Print WLSQ-uniform OBBT diagnostic line (rank 0 only)
        if rank == 0:
            diag = getattr(current_node, '_wlsq_obbt_diag', None)
            if diag:
                status = diag.get('status', 'OFF')
                shrink_pct = diag.get('shrink_pct')
                ratio = diag.get('ratio')
                tightened_count = diag.get('tightened_count', 0)
                
                if shrink_pct is not None:
                    print(f"    WLSQ_OBBT(uniform): shrink={shrink_pct:.1f}%  ratio={ratio:.3f}  tightened={tightened_count}  status={status}")
                else:
                    print(f"    WLSQ_OBBT(uniform): shrink=-  ratio=-  tightened={tightened_count}  status={status}")
        ...
        if self.collect_plot_info and rank==0:
            ...
            # Record original SNoGloDe LB/UB for the CURRENT NODE (for fair comparison)
            # We want to compare WLSQ bound for node X vs Original bound for node X
            orig_lb = current_node.lb_problem.original_objective
            orig_ub = self.tree.metrics.ub # UB is always global
            
            self.wlsq_hist["original_lb"].append(float(orig_lb) if orig_lb is not None and math.isfinite(orig_lb) else np.nan)
    ...
    def display_status(self, 
        ...
        main_outputs = [round(self.runtime,3),
                   ...
                   f"{self.tree.metrics.absolute_gap:.2e}",
                   self.tree.n_nodes(),
                   method_str]
