...
def compute_wls_quadratic_surrogate_bound(
    ...
    if on_U:
        ...
        if math.isfinite(min_Q_bar):
            ...
            # Store surrogate dict for WLSQ OBBT module
            # beta layout: [d, c[0], c[1], ..., c[dim-1], Q[0,0], Q[0,1], ..., Q[dim-1,dim-1] upper triangular]
            # Reconstruct Q matrix (symmetric) and c vector from g_beta_uni
            Q_obbt = np.zeros((dim, dim))
            c_obbt = g_beta_uni[1:dim+1].copy()
            d_obbt = g_beta_uni[0]
            quad_idx = dim + 1
            for i in range(dim):
                for j in range(i, dim):
                    val = g_beta_uni[quad_idx]
                    if i == j:
                        Q_obbt[i, i] = val
                    else:
                        # beta stores x[i]*x[j] coefficient directly; split for symmetric Q
                        Q_obbt[i, j] = val / 2.0
                        Q_obbt[j, i] = val / 2.0
                    quad_idx += 1
            
            # Store var_ids in order [Kp, Ki, Kd]
            var_ids_for_obbt = [lv['id'] for lv in lifted_vars]
            
            # Add mbar shift to d_obbt so phi(x) = x^T Q x + c^T x + d is a valid LB
            # This makes the stored surrogate represent the SHIFTED lower bounding function
            d_obbt_shifted = d_obbt + mbar_uniform
            
            node.lb_problem.wlsq_surrogate = {
                'Q': Q_obbt,
                'c': c_obbt,
                'd': d_obbt_shifted,
                'var_ids': var_ids_for_obbt
            }
