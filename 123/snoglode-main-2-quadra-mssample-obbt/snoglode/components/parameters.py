...
class SolverParameters():
    def __init__(self, 
        ...
        # WLSQ Surrogate OBBT parameters
        self._wlsq_obbt = False
        self._wlsq_obbt_delta = 1e-8
        self._wlsq_obbt_only_convex = True
        self._wlsq_obbt_frequency = 1
        self._wlsq_obbt_solver_name = "gurobi"
        self._wlsq_obbt_solver_opts = {}
    ...
    def set_wlsq_obbt(self,
                      enabled: bool = True,
                      delta: float = 1e-8,
                      only_convex: bool = True,
                      frequency: int = 1,
                      solver_name: str = "gurobi",
                      solver_opts: dict = None) -> None:
        """
        Configure WLSQ Surrogate OBBT (Objective-Based Bounds Tightening).
        ...
        ----------
        enabled : bool
            Enable WLSQ surrogate OBBT.
        delta : float
            Safety margin for constraint phi(x) <= UB - delta.
        only_convex : bool
            If True, only apply when surrogate Hessian is PSD (convex).
            If False, attempt nonconvex QCP solves.
        frequency : int
            Run OBBT every k iterations.
        solver_name : str
            Name of solver to use for OBBT QCPs.
        solver_opts : dict
            Solver options for OBBT QCPs.
        """
        assert type(enabled) == bool
        assert type(delta) == float and delta >= 0
        assert type(only_convex) == bool
        assert type(frequency) == int and frequency >= 1
        assert type(solver_name) == str
        
        self._wlsq_obbt = enabled
        self._wlsq_obbt_delta = delta
        self._wlsq_obbt_only_convex = only_convex
        self._wlsq_obbt_frequency = frequency
        self._wlsq_obbt_solver_name = solver_name
        self._wlsq_obbt_solver_opts = solver_opts if solver_opts else {}
