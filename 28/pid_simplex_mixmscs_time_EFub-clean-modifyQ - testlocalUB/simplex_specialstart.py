...
def run_pid_simplex_3d(base_bundles, ms_bundles, model_list, first_vars_list,
    ...
    * if you see verbose, ignore it, it just prints more things...
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Time (s)", "# Nodes", "LB", "UB", "Rel. Gap", "Abs. Gap", "best_LB_ever", "best_UB_ever", "LB_in_split", "UB_in_split", "LB_selected", "Rel_Gap_selected"])
    ...
    while len(nodes) < target_nodes:
        ...
        # 5) LB_global + simplex selection (split the simplex containing UB_node)
        _t_phase = perf_counter()
        # --- Global LB is still min over all active (or all) simplex LBs ---
        ...
        # --- Select simplex for splitting: the one containing UB_node ---
        _ub_pt_sel = np.asarray(UB_node, dtype=float)
        _ub_containing_recs = []
        _geom_tol_sel = 1e-8
        for _r in per_tet:
            _V = np.array(_r["verts"], dtype=float)  # (4, 3)
            _T_sel = (_V[1:] - _V[0]).T  # (3, 3)
            try:
                _lam123 = np.linalg.solve(_T_sel, _ub_pt_sel - _V[0])
                _lam0 = 1.0 - _lam123.sum()
                _lam_all = np.array([_lam0, *_lam123])
                if np.all(_lam_all >= -_geom_tol_sel):
                    _ub_containing_recs.append(_r)
            except np.linalg.LinAlgError:
                pass  # degenerate simplex, skip

        if _ub_containing_recs:
            # Among simplices containing UB_node, pick the one with lowest LB
            lb_simp_rec = min(_ub_containing_recs, key=lambda r: r["LB"])
            if verbose:
                _cont_ids = [r["simplex_index"] for r in _ub_containing_recs]
                print(f"[Iter {it}] UB-simplex selection: UB_node in simplices {_cont_ids}, "
                      f"chose T{lb_simp_rec['simplex_index']} (LB={lb_simp_rec['LB']/S:.9f})")
        else:
            # Fallback: UB_node outside mesh (e.g. from EF solve) -> use min-LB simplex
            if active_LBs:
                lb_simp_rec = min(
            ...
            if verbose:
                print(f"[Iter {it}] UB-simplex selection: UB_node OUTSIDE mesh, "
                      f"fallback to min-LB simplex T{lb_simp_rec['simplex_index']} "
                      f"(LB={lb_simp_rec['LB']/S:.9f})")
        ...
        # === NEW: LB of the selected (split) simplex and relative gap based on it ===
        if selected_rec_before_split is not None:
            _lb_selected = float(selected_rec_before_split["LB"]) / S
            _rel_gap_selected = (ub_val - _lb_selected) / (abs(ub_val) + 1e-16)
            _lb_selected_str = f"{_lb_selected:.9f}"
            _rel_gap_selected_str = f"{_rel_gap_selected*100:.7f}%"
        else:
            _lb_selected_str = "N/A"
            _rel_gap_selected_str = "N/A"

        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([f"{iter_time:.3f}", n_nodes, f"{lb_val:.9f}", f"{ub_val:.9f}", f"{rel_gap*100:.7f}%", f"{abs_gap:.5f}", f"{lb_ever:.9f}", f"{ub_ever:.9f}", _lb_in_split_str, _ub_in_split_str, _lb_selected_str, _rel_gap_selected_str])
